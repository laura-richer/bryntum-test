        ...
        * calculateCriticalPaths () : ChronoIterator<CriticalPath[]> {
            ...
            while ((path = pathsToProcess.shift())) {
                ...
                while ((node = path[taskIndex])) {
                    ...
                    const dependencies : SchedulerProDependencyMixin[] = []

                    let event = node.event

                    // collect the node incoming dependencies (including the inherited from its parents)
                    while (event && !event.isRoot) {
                        dependencies.push(...(yield event.$.incomingDeps))
                        event = event.parent
                    }
