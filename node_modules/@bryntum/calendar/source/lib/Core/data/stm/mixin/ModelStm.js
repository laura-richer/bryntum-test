...
export default Target => class ModelStm extends (Target || Base) {
    ...
    beforeChronoFieldSet(fieldName, value) {
        ...
        if (!me.inSetting && me.stm?.enabled && !unrecordedFields[fieldName] && !me.constructor.nonPersistableFields[fieldName]) {
            ...
            if (me.getFieldDefinition(fieldName)) {
                // Some fields may be set inside the engine passing the STM logic. For instance, when setting a
                // constraint type on a task constraint date atom is marked as changed and next time constraintDate is
                // read, its value gets calculated. Thus store will be under impression that value was already set and
                // is not modified in the current transaction. Therefore, for chrono fields we are reading current
                // values from the engine.
                // This is indirectly covered by a revision test. To see it in action replace it with `me[fieldName]`,
                // set constraint type on task and check transaction action queue. It will only contain constraint type
                // change.
                return { [fieldName] : { value, oldValue : me.graph.activeTransaction.readCurrentOrProposedOrPrevious(me.$[fieldName]) } };
            }
        }
    ...
    afterSet(field, value, silent, fromRelationUpdate, wasSet, isChronoFieldSet) {
        ...
        if (stm?.isBase && stm.shouldRecordAction && !unrecordedFields[field] && !nonPersistableFields[field]) {
            if (wasSet) {
                let shouldRecord;
    ...
    beforeRemoveChild(childRecords, isMove) {
        ...
        if (stm?.shouldRecordAction && !isMove) {
