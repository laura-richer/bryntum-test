...
export default Target => class GridSubGrids extends (Target || Base) {
    ...
    changeSubGridConfigs(configs) {
        ...
        if (configs.normal?.width) {
            delete configs.normal.flex;
        }
    ...
    createSubGrid(region, config = null) {
        const
            me             = this,
            subGridColumns = me.columns.chain(column => column.region === region, ['region']),
            subGridConfig  = SubGrid.mergeConfigs({
                type        : 'subgrid',
        ...
        if (!subGridConfig.flex && !subGridConfig.width && subGridConfig.flex !== 0 && subGridConfig.width !== 0) {
