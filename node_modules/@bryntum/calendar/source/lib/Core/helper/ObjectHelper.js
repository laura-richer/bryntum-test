...
const
    ...
    { hasOwn } = Objects,
    firstOfAsync = async(iter, defaultValue) => {
        let ret = defaultValue;

        // noinspection LoopStatementThatDoesntLoopJS
        for await (ret of iter) {   // eslint-disable-line no-unreachable-loop
            break;
        }

        return ret;
    },
    toFixedFix = (1.005).toFixed(2) === '1.01' ? null : (number, fractionDigits) => {
        // Browser has bad implementation of toFixed()
        const
...
export default class ObjectHelper extends Objects {
     ...
     *
     * @param {Object|String|String[]} source Source of keys to gather into an array.
     ...
     * @param {Function} fn The function to call with each property and value of the given `object`.
     * @param {*} fn.value The value of the `object` property `name`.
     * @param {String} fn.name The name of the current property for the iteration.
     */
    static forEach(object, fn) {
        let key;
        ...
        if (object) {
            if (Array.isArray(object)) {
                ...
                for (key in object) {
                    // pass (value, key) to match Array.forEach
                    fn(object[key], key);
                }
            }
        }
     ...
     * @param {Boolean} [useIsDeeply=false] Pass `true` to match arrays & objects using a deep equality check.
    ...
    static isEqual(a, b, useIsDeeply = false, matchNullish = false) {
        ...
        if (typeA === typeB) {
            ...
            switch (true) {
                case a.isModel && b.isModel:
                    // Comparing only model data
                    return a.equals(b);

                case a.isBase && b.isBase:
                    // Skip comparing object instances
                    return a === b;
                ...
                case typeA === 'object' && a.constructor.prototype === b.constructor.prototype:
                    return useIsDeeply ? OH.isDeeplyEqual(a, b, useIsDeeply) : Helpers.String.safeJsonStringify(a, 'circular') === Helpers.String.safeJsonStringify(b, 'circular');
            }
        }
    ...
    static isLessThan(a, b) {
        if (a instanceof Date && b instanceof Date) {
            return Helpers.Date.isBefore(a, b);
        }
        return a < b;
    }
    ...
    static isMoreThan(a, b) {
        if (a instanceof Date && b instanceof Date) {
            return Helpers.Date.isAfter(a, b);
        }
        return a > b;
    }

    /**
     * If `value` is iterable (`for-of` loopable), this method returns the first element of the iterable sequence.
     * If `value` is async-iterable (i.e., uses a `for await of` loop), this method returns a `Promise` that will
     * resolve to the first element.
     * Otherwise, this method returns the first key/value pair (a 2-element array) using a `for-in` loop.
     * @param {*} value
     * @param {*} [defaultValue] The value to return if `value` is empty.
     * @returns {*}
     * @internal
     */
    static first(value, defaultValue) {
        let ret = defaultValue,
            key;
        ...
        if (value) {
            if (value[Symbol.iterator]) {
                // noinspection LoopStatementThatDoesntLoopJS
                for (ret of value) {     // eslint-disable-line no-unreachable-loop
                    break;
                }
            }
            else if (value[Symbol.asyncIterator]) {
                ret = firstOfAsync(value, defaultValue);
            }
            else if (ObjectHelper.isObject(value)) {
                if (ret === undefined) {
                    ret = EMPTY_ARRAY;
                }

                // noinspection LoopStatementThatDoesntLoopJS
                for (key in value) {     // eslint-disable-line no-unreachable-loop
                    ret = [key, value[key]];
                    break;
                }
            }
     ...
     * @param {Object} dest The destination into which properties are copied.
     * @param {Object} source The source from which properties are copied.
     * @param {Object} except Object whose keys specify the properties to exclude.
     * @returns {Object} The `dest` object.
     */
    static copyPropertiesExcept(dest, source, except) {
        if (source) {
    ...
    static coerce(from, to) {
        ...
        if (fromType !== toType) {
            switch (toType) {
                ...
                case 'date':
                    return isString && isNaN(from) ? Helpers.Date.parse(from) : Date(Number(from));
            }
        }
        return from;
    }
     ...
     * @param {String} name Name of the value, used in error message
     */
    static assertNumber(value, name, positive) {
        const asNumber = Number(value);

        if (typeof value !== 'number' || isNaN(asNumber) || (positive && value < 0)) {
            throw new Error(`Incorrect type for ${name}, got "${value}" (expected a ${positive ? 'positive ' : ''}Number)`);
        }
    }
