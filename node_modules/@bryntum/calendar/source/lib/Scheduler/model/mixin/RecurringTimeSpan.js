...
/**
 * This mixin class provides recurrence related fields and methods to a {@link Scheduler.model.TimeSpan} model. This
 * class is used to support recurring events (and time ranges) in the Scheduler and Calendar components.
 *
 * ## Data structure
 *
 * The mixin introduces two types of timespans: the __recurring timespan__ and its __occurrences__.
 * A __recurring timespan__ is a timespan having a {@link #field-recurrenceRule} specified and
 * its __occurrences__ are "fake" dynamically generated timespans. You can also provide {@link #field-exceptionDates}
 * to the data to exclude certain dates. See sample data below,
 *
 * ```javascript
 * {
 *     id: 7,
 ...
 *
 * To query the ocurrences for a specific timespan, use the {@link #function-getOccurrencesForDateRange} method:
 *
 * ```
...
export default Target => class RecurringTimeSpan extends (Target || TimeSpan) {
    ...
    /**
     * Override of {@link Core/data/Model}'s method. If an {@link #property-isOccurrence} is passed, it is detached from
     * its parent recurring event. If it still has a recurrence then the recurring event is changed to stop at the
     * occurrence date. If it has no recurrence an exception is added at the occurrence date.
     * @category Recurrence
     */
    remove() {
    ...
    get eventStore() {
        ...
        // store with recurrence mixin, we should return own store instead
    ...
    removeOccurrence(dateOrTimeSpan, eventStore = this.eventStore) {
        ...
        // Clear the occurrences *if* we are in an EventStore.
    ...
    sanitizeRecurrenceData(recurrence = this.recurrence) {
        // We must remove all occurrences whenever the recurrence changes
    ...
    buildOccurrence(occurrenceDate, isFirst) {
        ...
        if (!occurrence) {
            ...
            if (isFirst || onStartDate) {
                ...
                // But if our start time is not as the rule requires, adjust ourselves
                // This must not cause the recurrence record to be synced with this.
                if (!onStartDate && me.recurrence?.timeZone == null) {
                    me.preventRecurrenceSync = true;

                    // We are setting this flag here to make sure we don't invalidate a recurring record in Engine when
                    // data is still being written to the records
                    me._preventInvalidation  = me.project?.isWritingData;
                    me.setStartEndDate(occurrenceDate, occurrenceEndDate, true);

                    // Resetting the flag
                    me._preventInvalidation  = false;
                    me.preventRecurrenceSync = false;
    ...
    afterChange(toSet, wasSet, silent, ...args) {
        ...
        if (me.isOccurrence) {
            ...
            // If it's a recurring base that's in the store (Not a tentative, dragging event)
            // and is being rescheduled, ensure that its BYXXX rule modifiers
            // match the new reality if configured to do so.
            if (!me.preventRecurrenceSync && ('startDate' in wasSet) && me.isRecurring && eventStore?.autoAdjustRecurrence) {
                const
                    { recurrence } = me,
                    { startDate }  = me,
                    oldStartDate   = wasSet.startDate.oldValue,
                    oldMonthDate   = startDate.getDate(oldStartDate),
                    oldMonthWeek   = Math.ceil(oldMonthDate / 7),
                    ...
                    days           = recurrence.days?.slice(),
                    months         = recurrence.months?.slice(),
                    monthDays      = recurrence.monthDays?.slice(),
                    positions      = recurrence.positions?.slice();

                // We will update multiple fields, the rule must not be re-evaluated until we are finished
                recurrence.beginBatch();

                // The change to a new date clashes with one of our occurrences.
                // The filter needs to be there to filter out iterator bugs.
                if (!me.getOccurrencesForDateRange(startDate, DateHelper.add(startDate, 1)).filter(r => !(r.startDate - startDate))?.length) {
                    // Moved into empty space.
                    // If we had explicit days and monthDays, the old one needs to be replaced by the new one
                    // BYDAY=
                    if (days?.length) {
                        ArrayHelper.remove(days, RecurrenceDayRuleEncoder.encodeDay(oldStartDate.getDay()));
                        days.push(dayName);
                        recurrence.days = days.sort();
                    }
                    // BYMONTHDAY=
                    if (monthDays?.length) {
                        ArrayHelper.remove(monthDays, oldStartDate.getDate());
                        monthDays.push(monthDate);
                        recurrence.monthDays = monthDays.sort();
                    }
                    // BYSETPOS=
                    if (positions?.length) {
                        ArrayHelper.remove(positions, oldMonthWeek);
                        positions.push(monthWeek);
                        recurrence.positions = [...new Set(positions)].sort();
                    }
                    // If we dragged the base out of its start month, we have to
                    // remove the old start month and add the new one (But dedupe it using Set)
                    if (months?.length) {
                        ArrayHelper.remove(months, oldStartDate.getMonth() + 1);
                        months.push(monthNumber);
                        recurrence.months = [...new Set(months)].sort();
                    }
                }

                // Flush changes which will update our recurrenceRule property
                // with its callback to our onRecurrenceChanged
                recurrence.endBatch();
            }
            ...
            // Clearing the recurrenceRule should also clear exceptionDates
            if (wasSet.recurrenceRule && !wasSet.recurrenceRule.value) {
                me.meta.modified.exceptionDates = true;
                me.exceptionDates = {};
            }
        }
     ...
     * The method adds an entry to the array kept in {@link #field-exceptionDates} field.
     ...
     */
    hasException(date) {
        return Boolean(this.exceptionDates?.[DateHelper.makeKey(date)]);
    }
};
