...
function cloneAttributesAndChildren(node) {
    ...
    if (node.nodeType === 1) {
        ...
        ArrayHelper.asArray(children).forEach(child => newNode.appendChild(child));
    }
    if (node.nodeType === 3) {
...
class DomHelperOverride {
    ...
    static getRootElement(element) {
        ...
        // our content
        return root?.body || root?.querySelector('.bryntum-root-element') || root?.firstChild || root || element.ownerDocument.body;
    }
