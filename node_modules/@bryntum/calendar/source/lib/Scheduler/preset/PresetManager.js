...
class PresetManager extends PresetStore {
                    ...
                    headers : [
                        {
                            unit : 'week',
                            renderer(start) {
                                if (DateHelper.weekStartDay === 1) {
                                    const
                                        [isoYear, isoWeek] = DateHelper.getWeekNumber(start),
                                        // Get the date for Thursday in that ISO week
                                        thursdayOfISOWeek  = DateHelper.getDateFromISOWeek(isoYear, isoWeek, 4), // 4 = Thursday
                                        monthName          = DateHelper.format(thursdayOfISOWeek, 'MMM');

                                    return `${DateHelper.getShortNameOfUnit('week')}.${String(isoWeek).padStart(2, '0')} ${monthName} ${isoYear}`;
                                }
                                else {
    ...
    set basePresets(basePresets) {
        ...
        // createRecord uses _modelClass for performance reasons, make sure it is available
    ...
    set defaultPresets(defaultPresets) {
        for (let i = 0, { length } = defaultPresets; i < length; i++) {
            ...
            // If it's an object, it's an adjustment to a base type
                ...
                const
                    config             = Object.assign(Object.create({ id : baseType }), ObjectHelper.clone(this.basePresets[baseType].data)),
