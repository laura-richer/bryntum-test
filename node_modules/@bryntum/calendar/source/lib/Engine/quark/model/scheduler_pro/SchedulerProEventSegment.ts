    ...
    class SchedulerProEventSegment extends base {
        ...
        writeStartDate (me : Identifier, transaction : Transaction, quark : Quark, date : Date, keepDuration : boolean = true) {
            ...
            if (event && !this.previousSegment && transaction.baseRevision.hasIdentifier(me) && !(project && project.getStm().isRestoring)) {
                transaction.write(event.$.startDate, date, keepDuration)
            }
            else {
            ...
            if (keepDuration && this.nextSegment) {
                ...
                if (shift > 0) {
                    ...
                    while ((segment = segment.nextSegment)) {
                        transaction.write(segment.$.startOffset, segment.startOffset + shift)
                        transaction.write(segment.$.endOffset, segment.endOffset + shift)
                    }
                }
            }
        }
        ...
        * calculateEndOffsetByMasterDurationAndStartOffset () : CalculationIterator<Duration> {
            ...
            let masterDurationMs : Duration   = yield* this.convertDurationGen(masterDuration, masterDurationUnit, TimeUnit.Millisecond)
        ...
        * calculateStartOffsetByEndOffsetAndDuration () : CalculationIterator<Duration> {
            ...
            return endOffset - (yield* this.event.convertDurationGen(duration, durationUnit, TimeUnit.Millisecond))
        }
        ...
        * calculateEndOffsetByStartOffsetAndDuration () : CalculationIterator<Duration> {
            ...
            return startOffset + (yield* this.event.convertDurationGen(duration, durationUnit, TimeUnit.Millisecond))
        }
        ...
        * calculateDurationByOffsets () : CalculationIterator<Duration> {
            ...
            return yield* this.convertDurationGen(endOffset - startOffset, TimeUnit.Millisecond, durationUnit)
        }
        ...
        * calculateManuallyScheduled () : CalculationIterator<boolean> {
            if (this.event) {
                return yield* this.event.isManuallyScheduled()
            } else {
                return yield ProposedOrPrevious
            }
        }
        ...
        afterChange (toSet, wasSet, silent, fromRelationUpdate, skipAccessors) {
            ...
            if (wasSet && event) {
                if (!('segments' in event.meta.modified) && event.segments) {
                    const prevSegmentsValue     = event.segments.map(segment => Object.assign(segment.toJSON(), segment.meta.modified))

                    event.storeFieldChange('segments', prevSegmentsValue)
                }

                // HACK: the `applyingSyncResponse` is a CrudManager property
                // we override it to `true` for the purpose of not silencing the events
                // when calling `event.afterChange` below
                const project                   = event.getProject() as SchedulerProProjectMixin & { applyingSyncResponse : boolean }

                const prev                      = project.applyingSyncResponse
                project.applyingSyncResponse    = true

                const newSegmentsValue          = event.segments?.map(segment => segment.toJSON())

                event.afterChange(
                    { segments : newSegmentsValue },
                    { segments : newSegmentsValue },
                    // this flag is forced to `true` in the `ChronoPartOfProjectStoreMixin.ts`
                    // we work around with `applyingSyncResponse`
                    silent,
                    fromRelationUpdate, skipAccessors
                )

                project.applyingSyncResponse    = prev
            }
        }
    }
