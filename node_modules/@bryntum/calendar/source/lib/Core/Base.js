...
class ConfigBinding {
    constructor(source, target, mapping, options) {
        const me = this;
        ...
        me.syncing = false;
        me.target = target;
        me.sourceHook = Helpers.Function.after(
            source, 'onConfigChange', info => me.onConfigChange(info, true), me, noReturn);
        me.targetHook = options?.oneway ? null : Helpers.Function.after(
            target, 'onConfigChange', info => me.onConfigChange(info, false), me, noReturn);
        ...
        me.unbind = me.destroy.bind(me);
        me.unbind.binding = me;
    }
    ...
    onConfigChange(info, fromSource) {
        const
            me = this,
            map = fromSource ? me.mapFwd : me.mapRev;
    ...
    sync(fromSource) {
        fromSource = fromSource !== false;

        const
            me = this,
            [from, to, map] = fromSource ? [me.source, me.target, me.mapFwd] : [me.target, me.source, me.mapRev];

        if (!me.syncing && !from.isDestroying && !to.isDestroying) {
            let n = 0,
                changes, fk, fv, tk, key, val;

            // if we are inside setConfig, reading the properties will flush any pending setters (and will likely call
            // here again), so flag ourselves as busy immediately.
            me.syncing = true;
            ...
            for (fk in map) {
                ...
                if (fv !== to[tk]) {
                    ++n;
                    (changes || (changes = {}))[key = tk] = val = fv;
                    // [key, val] hold the last pair that was added to changes
                }
            }

            if (n > 1) {
                to.setConfig(changes);
            }
            else if (n) {
                to[key] = val;
            }
    ...
    getInherited(name, create = true) {
        ...
        if (!(name in this)) {
            ...
            if (ret || create) {
                ...
                defineProperty(ret, '$meta', { value : this });
            }
        }
...
const
    ...
    hasOwn                   = (hasOwn => (o, p) => hasOwn.call(o, p))(Object.prototype.hasOwnProperty),
    ...
    // Naming this clone() makes the static clone() method on other classes disappear from docs 😵‍💫
    cloneObject              = o => Helpers.Object.clone(o),
    emptyFn                  = () => {},
    newMeta                  = o => new MetaClass(o),
    isObject                 = o => Helpers.Object.isObject(o),
    noReturn                 = { return : false },
    setupNames               = { /* foo : 'setupFoo' */ },
    emptyObject              = Object.freeze({}),
    emptyArray               = Object.freeze([]),
    configTypeRe             = /^(date|day)(\[])?$/;
...
export default class Base {
    ...
    constructor(...args) {
        ...
        if (me.$meta.class !== C) {
            ...
            // prototype. Since that alone would be optimized away, we assign a value on it (used to use emptyFn, but
            // Angular optimizes that away).
            C.$meta.initialized = true;
        }
    ...
    static new(...configs) {
        ...
        configs = configs.length > 1 ? this.mergeConfigs(...configs) : configs[0];
     ...
     * @param {Number} duration The duration of the change in milliseconds.
     * @param {'linear'|'easeInBack'|'easeOutBack'|'easeInOutBack'|'easeOutBounce'|'easeInOutBounce'|'easeInCirc'|'easeOutCirc'|'easeInOutCirc'|'easeInCubic'|'easeOutCubic'|'easeInOutCubic'|'easeInElastic'|'easeOutElastic'|'easeInOutElastic'|'easeInExpo'|'easeOutExpo'|'easeInOutExpo'|'easeInQuad'|'easeOutQuad'|'easeInOutQuad'|'easeInQuart'|'easeOutQuart'|'easeInOutQuart'|'easeInQuint'|'easeOutQuint'|'easeInOutQuint'|'easeInSine'|'easeOutSine'|'easeInOutSine'} [easing='linear'] The easing function to use.
     * @category Misc
     */
    animateProperty(propertyName, newValue, duration = 1000, easing) {
        const
            clearing = newValue == null,
            oldValue = this[propertyName] || 0,
            delta    = (newValue || 0) - oldValue,
            result   = FunctionHelper.animate(duration, p => this[propertyName] = oldValue + delta * p, null, easing);

        if (clearing) {
            result.then(() => this[propertyName] = null);
        }
        return result;
    }
    ...
    resolveProperty(propertyPath) {
        ...
        while (thisObj) {
            if (Base.hasConfigPath(thisObj, propertyPath, true)) {
                return Helpers.Object.getPath(thisObj, propertyPath);
            }
    ...
    configure(config = {}) {
        ...
        const
            me                  = this,
            meta                = me.$meta,
            { beforeConfigure } = (me.initialConfig = config),
            { configs }         = meta,
    ...
    /**
     * Creates a binding between the specified config properties of this object and the specified `target` object.
     * Changes to these config properties of this object will be reflected to the `target` object as they occur, and
     * (optionally), vise-versa.
     *
     * Returns a function to call to remove the binding.
     *
     * @param {Core.Base} target
     * @param {Object} mapping An object whose keys are the config properties of this object to sync to the `target`.
     * Any key with a falsy value is ignored. Keys with a string value use that value as the name of the corresponding
     * config property of the `target`. Any other key with a truthy value is used as the name of the config property
     * for both this object and the `target`.
     * @param {Object} [options] Options controlling the sync process
     * @param {Object} [options.oneway=false] Pass `true` to disable syncing config property changes on the `target`
     * back to this object.
     * @param {Object} [options.sync=true] Pass `false` to disable the initial sync of config properties to the
     * `target` object.
     * @returns {Function}
     * @internal
     */
    bindConfigs(target, mapping, options) {
        const binding = new ConfigBinding(this, target, mapping, options);

        (options?.sync !== false) && binding.sync();
     ...
     *      inst.getConfig('foo');
     *
     * @param {String} name The name of a config to (initialize and) return.
     * @param {...String} names Additional config names to return. If more than one parameter is passed, an array of
     * config values is returned. Otherwise, only the config value for `name` is returned.
     * @return {*}
     * @internal
     * @category Configuration
     */
    getConfig(name, ...names) {
        ...
        return names.length ? [val, ...names.map(n => this[n])] : val;
    }

    /**
     * This is intended to set a block of configs *during* configuration. It is not intended to be used
     * outside of the configuration phase.
     *
     * If any of the properties are not yet ingested from the configuration object, they will be set
     * and ingested in the normal order.
     *
     * Properties which are already ingested are set immediately.
     * @param {Object} props  An object containing configurations to change.
     * @internal
     */
    updateConfiguration(props) {
        const
            ...
            { configDone } = me,
            { configs }    = me.$meta,
            configValues   = me[configuringSymbol],
            lazyValues     = me[lazyConfigsSymbol];
        ...
        for (const key in props) {
            if (configDone[key]) {
                ...
                if (configs[key]?.lazy) {
                    lazyValues.set(key, props[key]);
                }
                else {
                    configValues[key] = props[key];
                }
            }
        }
        me.recompose.resume();
     ...
     * @advanced
     */
    setConfig(config, isConstructing = false) {
        const
            me             = this,
        ...
        me[configuringSymbol] = wasConfiguring ? Object.assign(Object.create(wasConfiguring), config) : config;
        ...
        for (key in config) {
            ...
            // passed configuration or the default value is null.
            // When used at construct time, defaultConfigs
            // will be prototype-chained onto the config.
            if (config[key] !== null || (hasOwn(config, key) || cfg.default === null)) {
    ...
    static hasConfigPath(object, path) {
        return path.split('.').every(key => {
            ...
            const
                ...
                isConfig = config && config.descriptor.get === Helpers.Object.getPropertyDescriptor(object, key)?.get;
    ...
    getCurrentConfig(options = { }) {
        const
            me      = this,
            configs = options.configs === 'all' ? me.config : cloneObject(me.initialConfig),
            skip    = options.skip || { },
    ...
    getConfigString(options = {}) {
        return Helpers.String.toJavaScriptString(this.getCurrentConfig(options));
    }
    ...
    downloadTestCase(options = {}) {
        ...
        Helpers.Browser.download(`app.js`, 'data:application/javascript;charset=utf-8,' + escape(app));
    }
    ...
    static setupClass(meta) {
        ...
        for (let decl, setupName, i = 0; i < meta.declarables.length; ++i) {
            ...
            if (hasOwn(cls, decl)) {
                setupName = setupNames[decl] || (setupNames[decl] = `setup${Helpers.String.capitalize(decl)}`);
        ...
        for (const name in configs) {
            ...
            if (simple) {
                ...
                if (value && typeof value === 'object' && '$config' in value) {
                    ...
                    if (configTypeRe.test(String(options))) {  // date/date[]/day/day[]
                        options = {
                            type : options
                        };
                    }
                    ...
                    if (options) {
                        if (!isObject(options)) {
                            options = Helpers.Object.createTruthyKeys(options);
                        }
            ...
            if (value && (isObject(value) || Array.isArray(value)) && !Object.isFrozen(value)) {
    ...
    static fork(obj) {
        ...
        if (obj && isObject(obj) && !Object.isFrozen(obj)) {
    ...
    static getDefaultConfiguration() {
        // $meta ensures setupClass has been called
        return this.$meta.class.prototype.getDefaultConfiguration();
    }
    ...
    getDefaultConfiguration() {
        ...
        if (VersionHelper.isTestEnv && Helpers.Browser.isBrowserEnv &&
    ...
    getProperties() {
        ...
        for (let i = 0; i < hierarchy.length; i++) {
            ...
            Object.assign(result, desc.get !== undefined ? hierarchy[i].properties : cloneObject(desc.value));
        }
     ...
     *
     * This method allows you to define additional or override existing default property values
     * that apply to all instances of the class.
     *
     * ```javascript
     ...
     * });
     * ```
     * @param {Object<String, *>} defaultsObj An object with the default values to set.
     * @category Configuration
     * @advanced
     */
    static applyDefaults(defaultsObj) {
        // Set the defaults on the class's meta object
        for (const key in defaultsObj) {
            this.$meta.config[key] = defaultsObj[key];
        }
    }
     ...
     * @param {...String|Symbol} name The name(s) of the event listeners to be removed.
     * @category Events
     * @advanced
     */
    detachListeners(...names) {
        const { $detachers } = this;

        if ($detachers) {
            for (let i = 0, { length } = names, detacher; i < length; i++) {
                // eslint-disable-next-line no-cond-assign
                if (detacher /* assignment */ = $detachers[names[i]]) {
                    while (detacher.length) {
                        detacher.pop()();
                    }
                }
            }
...
// We cannot rely on Helpers.Browser/Version at file-scope
...
VersionHelper.setVersion('core', '6.2.2');
