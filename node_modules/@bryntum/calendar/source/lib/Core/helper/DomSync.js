...
export default class DomSync {
            ...
            if (getPrototypeOf(is) !== getPrototypeOf(was) || is instanceof Node) {
                ...
                if (equal) {
                    if (toString.call(was) === '[object Date]') {
                        ...
                        DomHelper.isReactElement(is) || DomHelper.isReactElement(was) ||
                        DomHelper.isVueConfig(is) || DomHelper.isVueConfig(was)
                    ) {
                        equal = false;
                    }
                    else {
    ...
    static insertElement(domConfig, targetElement, targetNode, refOwner, syncIdMap, syncId, options) {
        ...
        // elementData holds custom data that we want to attach to the element (not visible in dom)
        // Inject it before we sync to it in case connectedCallback needs it.
        if (domConfig.elementData) {
            newElement.elementData = domConfig.elementData;
        }
        ...
        if (!('role' in domConfig) && !domConfig.tabIndex && !domConfig['aria-hidden'] && !domConfig.alt && !DomHelper.isFocusable(newElement, true) && !newElement.htmlFor) {
    ...
    static syncChildren(options, targetElement) {
        ...
        while (sourceConfigs.length) {
            ...
            // React JSX, pass directly to callback (used by both React and Vue rendering)
            if (DomHelper.isReactElement(sourceConfig) || DomHelper.isVueConfig(sourceConfig)) {
            ...
            if (!isTextNode) {
                ...
                if (syncId != null && !sourceConfig.unmatched) {
                    ...
                    if (syncTargetElement) {
                        ...
                        if (syncTargetElement.retainElement && syncTargetElement.retainElement !== 'prevent-release') {
