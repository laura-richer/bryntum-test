...
const
     ...
     * @param {Object} [options.suspended] Pass `true` to start initially suspended.
    ...
    makeInvoker = (me, fn, wrapFn, options) => {
        ...
        wrapFn.calls = 0;
        wrapFn.suspended = options?.suspended;
        wrapFn.invoker = invoker;

        invoker.wrapFn = wrapFn;
        // These help inspection during debugging and can help w/conditional breakpoints:
        invoker.fn = fn;
        invoker.this = me;
    ...
    decorateWrapFn = (me, wrapFn, cancelFn = 'clearTimeout') => {
        ...
        wrapFn.suspended = wrapFn.suspended ? 1 : 0;
        wrapFn.timerId = null;
    ...
    raf(fn, options) {
        ...
        const
            rafWrapFn = (...params) => {
                ...
                if (rafWrapFn.cancelOutstanding || rafWrapFn.immediate) {
                    rafWrapFn.cancel();
                }
    ...
    idle(fn, options) {
        ...
        const
            idleWrapFn = (...params) => {
                ...
                // Reschedule the frame on each call if requested
                if (idleWrapFn.cancelOutstanding || idleWrapFn.immediate) {
                    idleWrapFn.cancel();
                }
        ...
        for (const name in delayable) {
            ...
            defineProperty(target, name, {
                get() {
                    const
                        me     = this,
                        caller = (...params) => me[implName](...params),
                        value  = me[options.type](caller, options);

                    // Allow the function to be replaced by FunctionHelper.before etc
                    defineProperty(me, name, { value, configurable : true, writable : true });
                    caller.methodName = name; // helpful for inspection and conditional breakpoints
