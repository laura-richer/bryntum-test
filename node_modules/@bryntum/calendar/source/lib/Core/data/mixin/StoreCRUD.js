...
export default Target => class StoreCRUD extends (Target || Base) {
    ...
    remove(records, silent = false) {
        ...
        if (me.isGrouped) {
            ...
            for (const [parent, children] of removals) {
                allRemovedRecords.push(...parent.removeChild(children, false, true, { skipIndexUpdate : true }));
                affectedParents.add(parent);
            }

            // Update child indices for all parents that had children removed, and are still in store
            for (const parent of affectedParents) {
                if (!allRemovedRecords.includes(parent)) {
                    parent.updateChildrenIndices(parent.children, 'parentIndex');
                    parent.unfilteredChildren && parent.updateChildrenIndices(parent.unfilteredChildren, 'unfilteredIndex');
                    parent.orderedChildren && parent.updateChildrenIndices(parent.orderedChildren, 'orderedParentIndex');
                }
            }
        ...
        if (me.storage.totalCount || me.rootNode?.unfilteredChildren?.length) {
            ...
            if (me.rootNode) {
                ...
                if (me.isChained && !me.isChainedTree) {
                    const allRecords = me.getAllDataRecords();
     ...
     *
     * {@note}If called on a lazy-loaded store, it removes all the loaded records. And if a backend is configured, then those deletions
     * will also be synced with the backend.{/@note}
    ...
    add(records, silent = false, options = {}) {
        ...
        if (me.tree) {
            const
                ...
                orderedIndexField = me.modelClass.getFieldDataSource('orderedParentIndex'),
            ...
            records.forEach(node => {
                indexField = indexField ||
                    ((!(parentIndexField in node) && (orderedIndexField in node)) ? orderedIndexField : parentIndexField);
                ...
                if (!node.isModel && indexField in node) {
            ...
            parentIdMap.forEach(({ append, insert }, parentId) => {
                ...
                if (insert.length) {
                    for (const toInsert of insert) {
                        // If we are inserting record with `orderedParentIndex` we need to also configure insertion to
                        // an ordered tree.
                        if ('orderedParentIndex' in toInsert) {
                            if ('orderedParentIndex' in options && ObjectHelper.isObject(options.orderedParentIndex)) {
                                options.orderedParentIndex.index = toInsert.orderedParentIndex;
                            }
                            else {
                                options.orderedParentIndex = toInsert.orderedParentIndex;
                            }
                        }

                        added.push(parentNode.insertChild(toInsert, toInsert[indexField], silent, options));
                    }
                }
            });
    ...
    // Helper fn used by processRecords() below and TreeNode.ingestChildren()
    cloneImmutable(records) {
        // Detect immutable data and clone it
        if (this.useRawData.enabled && !records[0].isModel && !Object.isExtensible(records[0])) {
            records = ObjectHelper.clone(records);
        }
        return records;
    }

    processRecords(records, onRecordCreated) {
        return this.cloneImmutable(records).map(data => {
    ...
    insert(index, records, silent = false) {
        const
            me            = this,
            storage       = me.storage,
            insertBefore  = storage.getAt(index),
    ...
    acceptChanges() {
        ...
        me.added.forEach(record => record.internalClearChanges(true, false));
