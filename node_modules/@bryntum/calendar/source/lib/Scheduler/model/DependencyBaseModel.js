 ...
 * @property {Number} StartToStart - Start-to-Start (`0`): The successor event cannot start before the predecessor starts
 * @property {Number} StartToEnd - Start-to-Finish (`1`): The successor event cannot finish before the predecessor starts
 * @property {Number} EndToStart - Finish-to-Start (`2`): The successor event cannot start before the predecessor finishes
 * @property {Number} EndToEnd - Finish-to-Finish (`3`): The successor event cannot finish before the predecessor finishes
 */
...
export default class DependencyBaseModel extends Model {
    static $name = 'DependencyBaseModel';
    ...
    static get fields() {
        return [
             ...
             * @field {DurationUnit|DurationUnitShort} lagUnit
             * @category Dependency
             * @readonly
             */
            {
    ...
    construct(data) {
        const
            ...
            typeFieldType = this.fieldMap.type.type;

        if (typeFieldType && typeFieldType !== 'int' && typeFieldType !== 'integer') {
            throw new Error('Dependency type field must be of type "int"');
        }
    ...
    get skipCopyFields() {
        const { fieldMap } = this;
        return {
            [fieldMap.from.dataSource]       : true,
            [fieldMap.to.dataSource]         : true,
            [fieldMap.fromEvent?.dataSource] : true,
            [fieldMap.toEvent?.dataSource]   : true,
            [fieldMap.fromTask?.dataSource]  : true,
            [fieldMap.toTask?.dataSource]    : true
        };
    }
}
