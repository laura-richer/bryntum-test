import { ProposedOrPrevious, ProposedOrPreviousValueOf, Write } from '../../../../ChronoGraph/chrono/Effect.js'
...
import { ConstraintIntervalSide, ConstraintType, Direction, StartedTaskScheduling } from '../../../scheduling/Types.js'
import { format } from '../../../util/Functions.js'
import { HasChildrenMixin } from '../scheduler_basic/HasChildrenMixin.js'
import { DependencyConstraintInterval } from "./ConstrainedByDependenciesEventMixin.js"
...
import { HasDateConstraintScheduleMixin } from "./HasDateConstraintScheduleMixin.js"
...
export class HasDateConstraintMixin extends Mixin(
    ...
    class HasDateConstraint extends base {
        ...
        // @ts-ignore
        static scheduleMixinClass  : typeof HasDateConstraintScheduleMixin   =
            Mixin(
                [ base.scheduleMixinClass, HasDateConstraintScheduleMixin ],
                base => base
            )
         ...
         */
        @model_field({ defaultValue : null, persist : false }, { sync : true })
        hasPostponedOwnConstraintConflict : ConflictEffect

        forceResolutionOfPostponedConflict : boolean      = false
        ...
        writeStartDate (me : Identifier, transaction : Transaction, quark : Quark, date : Date, keepDuration : boolean = true) {
            ...
            if (
                ...
                if (constrainType) {
                    // If it's not a branch (have to keep this code since some revision tests fail otherwise)
                    if (transaction.graph === this.graph) {
                    ...
                    // ideally this code should always run
                    else {
                        transaction.write(this.$.constraintDate, date)
                        transaction.write(this.$.constraintType, constrainType)
                    }
                }
            }
        ...
        writeDirection (me : Identifier, transaction : Transaction, quark : Quark, direction : Direction, fromWriteConstraintType : boolean = false) {
            ...
            if (!fromWriteConstraintType &&
                ...
                transaction.write(this.$.constraintType,
        ...
        * calculateConstraintDate (Y : SyncEffectHandler) : CalculationIterator<Date> {
        ...
        // Repeats this.isManuallyScheduled() generator method code but uses fields reading
        _isManuallyScheduled () : boolean {
            let manuallyScheduled : boolean = this.manuallyScheduled

            if (!manuallyScheduled) {
                const startedTaskScheduling : StartedTaskScheduling = this.project.startedTaskScheduling

                if (startedTaskScheduling === StartedTaskScheduling.Manual) {
                    manuallyScheduled = this.percentDone > 0
                }
            }
        ...
        isTaskPinnableWithConstraint () : boolean {
            const { ignorePinningConstraint, constraintType } = this

            let result = false

            // we should not pin manually scheduled tasks
            if (!this._isManuallyScheduled() && !ignorePinningConstraint) {
        ...
        applyChangeset (...args) : any {
            ...
            this.ignorePinningConstraint = true

            //@ts-ignore
            const result = super.applyChangeset(...args)
        ...
        * onNonEmptyEffectiveInterval (type : 'start' | 'end', sourceSchedule : ConstrainedScheduleMixin)
        ...
        {
            const isFirstPass : boolean                                 = yield* sourceSchedule.isFirstPass()
            const hasPostponedOwnConstraintConflict : ConflictEffect    = yield this.$.hasPostponedOwnConstraintConflict

            if (hasPostponedOwnConstraintConflict && isFirstPass && sourceSchedule.phase === hasPostponedOwnConstraintConflict.sourceSchedule.phase) {
                yield Write(this.$.hasPostponedOwnConstraintConflict, null)
            }
        ...
        async resolvePostponedConflict () : Promise<CommitResult> {
            if (this.hasPostponedOwnConstraintConflict) {
                // this is a reactive atom, assigning value to it will trigger re-calculations
                this.hasPostponedOwnConstraintConflict      = null
                // this is a regular "imperative" flag, which is used in calculations
                this.forceResolutionOfPostponedConflict     = true
            }
        ...
        * createStartDateConstraintsInterval (
            constraintType : ConstraintType, constraintDate : Date, schedule : ConstrainedScheduleMixin, intervals : DateInterval[]
        ) {
        ...
        * createEndDateConstraintsInterval (
            constraintType : ConstraintType, constraintDate : Date, schedule : ConstrainedScheduleMixin, intervals : DateInterval[]
        ) {
            const dateConstraintIntervalClass : typeof DateConstraintInterval = this.project.dateConstraintIntervalClass
...
export class RemoveDateConstraintConflictResolution extends Localizable(ConflictResolution) {
    ...
    static $name = 'RemoveDateConstraintConflictResolution'
    ...
    resolve () {
        ...
        this.event.constraintDate = null
    }
}
...
/**
 * Class implements resolving a scheduling conflict happened due to a task constraint.
 * It resolves the conflict by postponing it.
 */
export class PostponeDateConstraintConflictResolution extends Localizable(ConflictResolution) {

    static $name = 'PostponeDateConstraintConflictResolution'

    interval        : DateConstraintInterval


    get event () : HasDateConstraintMixin {
        return this.interval.owner
    }
     ...
     */
    resolve () {
        this.event.hasPostponedOwnConstraintConflict = this.conflict
    }
}
...
export class DateConstraintIntervalDescription extends ConstraintIntervalDescription {
    ...
    static $name = 'DateConstraintIntervalDescription'
    ...
    static getDescription (interval : DateConstraintInterval) : string {
        interval = interval.reflectionOf as DateConstraintInterval || interval
...
export class DateConstraintInterval extends ConstraintInterval {
     ...
     * The interval suggests the only resolution option - removing the constraint.
     */
    getResolutions (intervals : DateInterval[], conflict : ConflictEffect) : ConflictResolution[] {
        if (this.resolutions !== undefined) return this.resolutions

        const resolutions = [
            this.removeDateConstraintConflictResolutionClass.new({ interval : this })
        ]

        const project   = this.owner.getProject()

        if (
            (project.allowPostponedConflicts || project.autoPostponedConflicts)
            && this.owner.getOwnConstraintConflictInfo(intervals).kind !== 'no_own_constraint_conflict'
        ) {
            resolutions.push(PostponeDateConstraintConflictResolution.new({ interval : this, conflict }))
        }

        return this.resolutions = resolutions
    }
}
