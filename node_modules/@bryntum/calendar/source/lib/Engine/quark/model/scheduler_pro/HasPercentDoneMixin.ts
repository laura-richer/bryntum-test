...
import { ChronoIterator, CommitResult } from "../../../../ChronoGraph/chrono/Graph.js"
import { AnyConstructor, Mixin } from "../../../../ChronoGraph/class/BetterMixin.js"
import { calculate, field } from "../../../../ChronoGraph/replica/Entity.js"
import { model_field } from "../../../chrono/ModelFieldAtom.js"
import { Duration, StartedTaskScheduling, TimeUnit } from "../../../scheduling/Types.js"
...
export class HasPercentDoneMixin extends Mixin(
    ...
    class HasPercentDoneMixin extends base {
        ...
        setPercentDone : (value : number) => Promise<CommitResult>
        putPercentDone : (value : number) => void


        * isManuallyScheduled () : ChronoIterator<boolean> {
            const project = this.getProject() as SchedulerProProjectMixin

            let manuallyScheduled : boolean = yield this.$.manuallyScheduled

            const childEvents : Set<HasPercentDoneMixin> = yield this.$.childEvents

            // If that's not a manual task
            // and not a summary task
           if (!manuallyScheduled && !childEvents.size) {
                const startedTaskScheduling : StartedTaskScheduling = yield project.$.startedTaskScheduling
        ...
        * calculatePercentDone () : ChronoIterator<number> {
            ...
            if (childEvents.size && autoCalculatePercentDoneForParentTasks) {
                ...
                if (summaryData.totalDuration > 0) {
                    const milestonesCompletion = summaryData.milestonesTotalPercentDone / summaryData.milestonesNum
                    const completion =  summaryData.completedDuration / summaryData.totalDuration

                    return completion < 100 ? completion : summaryData.milestonesNum > 0 && milestonesCompletion < 100 ? 99 : completion
                }
        ...
        * calculatePercentDoneSummaryData () : ChronoIterator<PercentDoneSummaryData> {
            ...
            if (childEvents.size) {
                ...
                if (typeof duration == 'number') {
                    ...
                    const durationInMs : Duration   = yield* this.getDurationConverter().$convertDuration(duration, yield this.$.durationUnit, TimeUnit.Millisecond)
