...
export default class EditBase extends InstancePlugin {
    ...
    onDatesChange({ value, source }) {
        ...
        if (value && source === me.startDateField && me.startTimeField?.keepDate && me.startTimeField.value) {
            me.startTimeField.value.setFullYear(value.getFullYear());
            me.startTimeField.value.setMonth(value.getMonth());
            me.startTimeField.value.setDate(value.getDate());
        }
    ...
    async onSaveClick() {
        ...
        me.isFinalizingEventSave = true;

        const saved = await me.save();

        me.isFinalizingEventSave = false;

        if (saved) {
            await me.finalizeStmCapture(false);

            if (!me.isDestroyed) {
                // Pass this feature as a signal that its an active close, and not a cancel
                me.editor.close(me);
                 ...
                 * @param {'save'|'delete'|'cancel'} action The action that was taken to end the edit.
                 * One of 'save', 'delete' or 'cancel'
                 * @param {Scheduler.feature.EventEdit} eventEdit The eventEdit feature
                 * @param {Scheduler.model.EventModel} eventRecord The record that has been edited.
                 ...
                 */
                me.client.trigger('afterEventEdit', { ...me.editingContext, action : 'save' });
            }
        }
        return saved;
    }
    ...
    async onDeleteClick() {
        ...
        me.isDeletingEvent = true;

        const removed = await me.deleteEvent();

        me.isDeletingEvent = false;
        ...
        if (removed) {
            ...
            if (!me.isDestroyed) {
                ...
                me.client.trigger('afterEventEdit', { ...me.editingContext, action : 'delete' });
            }
        }
    }
    ...
    async onCancelClick() {
        ...
        me.isCancelingEdit = true;

        me.editor.close(me);

        me.isCancelingEdit = false;

        if (me.hasStmCapture) {
            await me.finalizeStmCapture(true);
        }

        if (!me.isDestroyed) {
            me.client.trigger('afterEventEdit', { ...me.editingContext, action : 'cancel' });
        }
    }
