...
export default class Layout extends Base.mixin(Events, Factoryable) {
    ...
    renderChildren() {
        ...
        if (itemCount) {
            ...
            for (let insertion, renderTo, i = 0; i < itemCount; i++) {
                const
                    item                      = items[i],
                    { element, positionable } = item,
                    { classList }             = element;
                ...
                renderTo = contentElement;

                if (positionable) {
                    insertion = owner.getPositionableLocation(positionable);

                    if (!(renderTo = insertion[0])) {
                        renderTo = insertion[1].parentElement;
                        item.insertBefore = insertion[1];
                    }
                }

                // Only trigger paint if the owner is itself painted, otherwise
                // the outermost Container will cascade the paint signal down.
                item.render(renderTo, Boolean(owner.isPainted));
            }
        }
    ...
    insertChild(toAdd, childIndex) {
        ...
        const isMove = contentElement.contains(element);
