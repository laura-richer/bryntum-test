        ...
        while ((!endDate || endDate > monthStartDate) && (!count || counter < count)) {
            ...
            if (weekDays?.length) {
                ...
                if (!hasPositions) {
                    for (i = 0; i < dates.length; i++) {
                        ...
                        if (occurrenceDate >= timeSpanStart) {
                            ...
                            const inView = this.isInView(startOnly, occurrenceDate, earliestVisibleDate, durationMS,
                                timeSpan);
                            ...
                            if (
                                // Stop if:
                                // - COUNT-limited and that value is collected
                                // - or UNTIL-limited and that date is reached
                                (count && counter > count) ||
                                (endDate && occurrenceDate >= endDate) ||
                                // otherwise if we are in the visible timespan
                                // run the fn-callback and stop if it returns FALSE
                                inView &&
                                    fn.apply(scope, [
                                        occurrenceDate,
                                        counter,
                ...
                for (i = 0; i < monthDays.length; i++) {
                    ...
                    if ((occurrenceDate = this.getNthDayOfMonth(monthStartDate, monthDays[i])) &&
                        !processedDate[occurrenceDate.getTime()]
                    ) {
                        processedDate[occurrenceDate.getTime()] = true;
