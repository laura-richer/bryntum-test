...
export default class TimeAxis extends Store {
    ...
    static get defaultConfig() {
        return {
            ...
            // we prevent using endless tick amount for performance reasons. We will recommend to reduce date range as an erorr in console
            maxAllowedTickCount : 10000
        };
    }
    ...
    reconfigure(config, suppressRefresh = false, preventThrow = false) {
        ...
        // If timeSpan exceed timeline boundaries, we try to set correct timespan.
        let
            normalized = me.getAdjustedDates(config.startDate, config.endDate),
            isChanged  = false;

        if (owner?.isBeforeMinDate(normalized.startDate)) {
            config.startDate = owner?.minDate;
            isChanged = true;
        }
        if (owner?.isAfterMaxDate(normalized.endDate)) {
            config.endDate = owner?.maxDate;
            isChanged = true;
        }
    ...
    getAdjustedDates(startDate, endDate, forceAdjust = false) {
        ...
        const
            startDateAdjusted   = me.floorDate(startDate, false, me.autoAdjust ? me.mainUnit : me.unit, 1),
            endDateAdjusted     = me.ceilDate(endDate, false, me.autoAdjust ? me.mainUnit : me.unit, 1),
            diffStartEndDates   = endDateAdjusted - startDateAdjusted,
            startDateAdjMinDate = owner?.getMinDateOrDate(startDateAdjusted) ?? startDateAdjusted;
        let endDateAdjMaxDate = owner?.getMaxDateOrDate(endDateAdjusted) ?? endDateAdjusted;

        if (endDateAdjMaxDate <= startDateAdjMinDate) {
            endDateAdjMaxDate = owner?.getMaxDateOrDate(DH.add(startDateAdjMinDate, diffStartEndDates));
        }

        return me.autoAdjust || forceAdjust ? {
            startDate : startDateAdjMinDate,
            endDate   : endDateAdjMaxDate
        } : {
            startDate,
            endDate
        };
    ...
    get maxAllowedEndDate() {
        const
            incrementDiff     = this.maxAllowedTickCount * DH.asMilliseconds(this.increment, this.unit),
            maxAllowedEndDate = DH.add(DH.parse(this.startDate), incrementDiff);
     ...
     */
    validateTickCount() {
        if (this.endDate > this.maxAllowedEndDate) {
            console.error('Configured date range will result in a too long time axis. If you required in long timeline, please use InfiniteScroll feature.');
        }
    }
