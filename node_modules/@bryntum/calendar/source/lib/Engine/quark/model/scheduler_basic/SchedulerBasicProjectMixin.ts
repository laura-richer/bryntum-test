...
import {
    ...
    model_field,
    ModelBucketField
} from "../../../chrono/ModelFieldAtom.js"
import { CycleEffect, EngineReplica } from "../../../chrono/Replica.js"
...
export class SchedulerBasicProjectMixin extends MixinAny(
    ...
    class SchedulerBasicProjectMixin extends base {
        ...
        construct (config : Partial<this> = {}) {
            ...
            this._queue = new Queue(IdHelper.generateId('ProjectModelQueue'))
        ...
        get dataStores () : ChronoPartOfProjectStoreMixin[] {
            return [
        ...
        get hasDataInStores () : boolean {
            return this.dataStores.some(store => store.totalCount > 0)
        }


        get hasChangesInStores () {
            return this.dataStores.some(store => store.hasChanges)
        }
        ...
        enterReplica (enterRecords : boolean) {
            ...
            if (enterRecords && !me.isRepopulatingStores) {
                ...
                me.dataStores.forEach(store => {
                    store.forEach(r => { !r.graph && r.joinProject() }, undefined, {
        ...
        doDestroy () {
            ...
            me.dataStores.forEach(store => store?.destroy())
        ...
        getReplicaConfig () : Partial<EngineReplica> {
            return {
                project                              : this,
                schema                               : Schema.new(),
        ...
        createReplica () : EngineReplica {
            return EngineReplica.new(this.getReplicaConfig())
        }
        ...
        * subEventsIterable () : CalculationIterator<Iterable<BaseEventMixin>> {
        ...
        async loadInlineData (data : ProjectDataPackage) : Promise<CommitResult> {
            ...
            if (data.project) {
                ...
                if (this.delayEnteringReplica && this.hasDataInStores) {
                    this.ion({
                        recordsUnlinked : () => {
                            // @ts-ignore
        ...
        async internalDelayCalculation (resolve : Function) {
            ...
            me.dataStores.forEach(store => store.suspendFilterOnUpdate())
            ...
            me.dataStores.forEach(store => store.resumeFilterOnUpdate())
        ...
        initializeStm () : void {
            ...
            // @ts-ignore
            if (!(this.stm instanceof StateTrackingManager)) this.stm = stmClass.new({ disabled : true }, this.stm)

            this.setStm(this.stm)
        ...
        removeRejectedRecordsAdd ({ transactionResult, silenceCommit } : { transactionResult : TransactionCommitResult, silenceCommit : boolean }) : void {
            const recordsToDrop = new Map<ChronoPartOfProjectStoreMixin, Set<ChronoPartOfProjectModelMixin>>()
            ...
            for (const quark of transactionResult.entries.values()) {
                ...
                const record    = identifier.self as ChronoPartOfProjectModelMixin
                const store     = record.firstStore as ChronoPartOfProjectStoreMixin
            ...
            if (silenceCommit) {
                this.dataStores.forEach(store => store.acceptChanges())
            }
        }
        ...
        async undo () {
            ...
            if (this._isCommitingAfterStmRestore) {
                await this.await('stateRestoringDone', false)
            }
        }
        ...
        _isCommitingAfterStmRestore : boolean

        async handleSTMRestoringStop (stm : StateTrackingManager) {
            if (this.replica) this.replica.isWritingPreviousData = false

            // Disable STM meanwhile to not pick it up as a new STM transaction
            stm.disable()

            this._isCommitingAfterStmRestore = true

            await this.commitAsync()

            this._isCommitingAfterStmRestore = false
        ...
        async onSTMRestoringStop ({ source : stm } : { source : StateTrackingManager }) {
            if (stm.revisionsEnabled) {
                await this.queue(() => this.handleSTMRestoringStop(stm))
            }
            else {
                await this.handleSTMRestoringStop(stm)
            }
        }
        ...
        repopulateStore (store : AbstractPartOfProjectStoreMixin) {
            ...
            if (me.repopulateOnDataset && store.totalCount && !store.syncDataOnLoad) {
        ...
        repopulateReplica () {
            ...
            calendarManagerStore.joinRecordsToProject(me, true)
            eventStore.joinRecordsToProject(me, true)
            dependencyStore.joinRecordsToProject(me, true)
            resourceStore.joinRecordsToProject(me, true)
            assignmentStore.joinRecordsToProject(me, true)

            if (oldReplica) {
                // copying the references failed to resolve from old replica
                // doing this only _after_ all data has been re-created,
                // since it maps by identifiers
                replica.activeTransaction.writeFailedResolutionInfo(oldReplica.baseRevision.failedResolutionReferences)
            }
        ...
        unlinkRecord (record : ChronoModelMixin) {
            record.unlink()
        }
        ...
        * calculateEffectiveDataOrVisualCalendar (purpose : 'data' | 'visual') : CalculationIterator<BaseCalendarMixin> {
        ...
        setEventStore (store : ChronoEventStoreMixin) {
            ...
            if (oldEventStore && this.stm.hasStore(oldEventStore)) {
                ...
                oldEventStore.unjoinRecordsFromProject(this)
            ...
            if (!store || !(store instanceof Store)) {
                ...
                if (store.tree && store.rootNode !== this) {
                    ...
                    store.joinRecordsToProject(this)
                }
            }
        ...
        setResourceStore (store : ChronoResourceStoreMixin) {
            ...
            if (oldResourceStore && this.stm.hasStore(oldResourceStore)) {
                ...
                oldResourceStore.unjoinRecordsFromProject(this)
        ...
        setAssignmentStore (store : ChronoAssignmentStoreMixin) {
            ...
            if (oldAssignmentStore && this.stm.hasStore(oldAssignmentStore)) {
                ...
                oldAssignmentStore.unjoinRecordsFromProject(this)
         ...
         */
        async tryPropagateWithChangesQueued (changerFn : Function) : Promise<boolean> {
            return this.queue(() => this.tryPropagateWithChanges(changerFn))
        }
        ...
        queue (fn : () => any, handleReject? : (error : Error | any) => any) : Promise<any> {
            ...
            return me._queue.queue(async function runQueueStep () {
         ...
         * @param calendar Calendar to find references to.
         * @returns Array of records referencing the provided calendar.
         */
        getCalendarConsumers (calendar : InstanceType<this['calendarModelClass']>) : HasCalendarMixin[] {
            const result = []

            if (this.calendar === calendar) result.push(this)

            result.push(...this.eventStore.query(record => record.calendar === calendar, true))
            result.push(...this.resourceStore.query(record => record.calendar === calendar, true))

            // collect nested calendar refs too
            if (calendar.children?.length) {
                for (const child of calendar.children) {
                    result.push(...this.getCalendarConsumers(child as InstanceType<this['calendarModelClass']>))
                }
            }
        ...
        unlinkCalendarConsumers (calendar : InstanceType<this['calendarModelClass']>) {
            const consumers = this.getCalendarConsumers(calendar)

            for (const consumer of consumers) {
                consumer.calendar = null
            }
        }
