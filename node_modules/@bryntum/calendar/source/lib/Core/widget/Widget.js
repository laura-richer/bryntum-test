...
const
    ...
    EMPTY                   = Object.freeze([]),
    NO_SYNC                 = Object.freeze({ sync : false }),
    floatRoots              = [],
    hideModes               = ['clip', 'display', 'opacity'],
    ...
    htmlRe                  = /<div|<button|<input|<p|<br|<li/,
    ...
    getEventTarget          = t => {
        if (t?.target) {
            const
                { ownerSVGElement } = t.target,
                svgRect             = ownerSVGElement?.getBoundingClientRect();

            // If in an SVG, make event relative to the SVG
            if (ownerSVGElement) {
                return {
                    target  : t.target.ownerSVGElement,
                    offsetX : t.clientX - svgRect.x,
                    offsetY : t.clientY - svgRect.y
                };
            }
 ...
 * @property {HTMLElement|Core.widget.Widget|Core.helper.util.Rectangle|Event} [target] The Widget or Element or Rectangle or event
 * to align to.
 * @property {Number|Number[]} [inflate] Arguments to {@link Core.helper.util.Rectangle#function-inflate}.
 ...
 * @property {Boolean|'min'} [matchSize] When aligning edge-to-edge, match the length of the aligned-to edge of the target.
 * This is only honored when `axisLock` is enabled and alignment succeeds on the requested axis.
 *
 * Setting this value to `'min'` means that the resulting aligned edge of this widget will *grow* to the size
...
export default class Widget extends Base.mixin(Localizable, Events, Delayable, Identifiable, Factoryable, KeyMap, RTL) {
    ...
    static get configurable() {
        return {
            ...
            /**
             * Determines how a widget will prevent itself from being seen when {@link #property-hidden} is set to
             * `true` or the {@link #function-hide} method is called.
             *
             ...
             * @returns {String}
             * @accepts {String|Function|DomConfig|DomConfig[]|VueConfig}
             * @category DOM
             */
            html : {
                $config : 'lazy',
             ...
             * ```
             *
             * @config {Object<String,String|null>}
             * @category Misc
             */
             ...
             *
             * @config {String|TooltipConfig|null}
             * @category Misc
             */
            tooltip : {
                $config : ['lazy', 'nullify'],
            ...
            /**
             * This config is used to absolutely position this widget in its parent {@link Core.widget.Container}. When
             * enabled, this widget will be rendered into the parent widget's primary {@link #property-element}. In
             * containers where the {@link #property-contentElement} is not the same as the `element` (for example,
             * {@link Core.widget.Panel}), the value of this config indicates if this widget should be rendered
             * `'before'` or `'after'` the `contentElement`.
             *
             * Because `positionable` widgets are `position: absolute` and may even be rendered outside of the owning
             * container's `contentElement`, these widgets do not participate in the standard layout of their container.
             *
             * It is not necessary to set this config explicitly when using the position configs {@link #config-top},
             *  {@link #config-right},  {@link #config-bottom}, or  {@link #config-left}, unless it needs to be set to
             *  `'after'`.
             *
             * @config {'before'|'after'|Boolean}
             * @default
             * @category Float & align
             * @internal
             */
             ...
             * {@link Core.widget.Container#property-items} of a Container), this config will be ignored. In this case
             * the owner is __always__ the encapsulating Container.
             *
             ...
             * {@link #event-focusOut} event, and the {@link #property-containsFocus} property remains set.
             *
            ...
            /**
             * The `top` CSS absolute position for this widget. If specified, the widget is implicitly configured as
             * {@link #config-positionable}.
            ...
            /**
             * The `inset-inline-end` CSS absolute position for this widget. If specified, the widget is implicitly
             * configured as {@link #config-positionable}.
             * @config {Number|String}
             * @internal
             */
            right : null,
            ...
            /**
             * The `bottom` CSS absolute position for this widget. If specified, the widget is implicitly configured as
             * {@link #config-positionable}.
             * @config {Number|String}
             * @internal
             */
            ...
            /**
             * The `inset-inline-start` CSS absolute position for this widget. If specified, the widget is implicitly
             * configured as {@link #config-positionable}.
             * @config {Number|String}
             * @internal
             */
            ...
            localizableProperties : ['title', 'ariaLabel', 'ariaDescription', 'dataset.text'],
            ...
            /**
             * A mapping object describing config properties to sync with the `overflowTwin`. This is pass to
             * {@link Core.Base#function-bindConfigs}.
             *
             * For example:
             * ```javascript
             *  twinSyncConfigs : {
    ...
    static get prototypeProperties() {
        return {
            ...
            /**
             * This property is set to `true` after processing the initial paint for the widget. It remains `false`
             * during the initial paint. The intended use for this flag is to avoid processing that will be handled by
             * the first paint (similar to not firing events during the widget's initial configuration). If this field
             * is `true`, the initial paint has already taken place, otherwise, it has yet to run. This field differs
             * from `isPainted` which checks that the widget's element is attached to the DOM.
             * @member {Boolean} hasPainted
             * @internal
             * @readonly
             * @category DOM
             */
    ...
    updateWeight() {
        if (this.isPainted) {
            this.parent?.onChildWeightChange?.(this);
        }
    }

    isA(type) {
        return this.isType(type, true);
    }
    ...
    static weightSortFn({ weight : a }, { weight : b }) {
     ...
     * @function create
     * @static
     * @param {ContainerItemConfig} config Config object with `type` string property
     * @typings config -> {ContainerItemConfig|Object}
     * @returns {Object} The instance of the created Widget
     * @category Lifecycle
     */
    ...
    finalizeInit() {
        const
            ...
            { classList } = me.element;

        // Ingest lazy content configs
        me.getConfig('html', 'content');

        if (refElement) {
            if (adopt) {
                classList.remove('b-hidden');
                me._hidden = false;
            }
        ...
        // Enable transitions after all rendering and onPaint processing done
        classList.remove('b-initializing');
    }
    ...
    doDestroy() {
        ...
        // The owner config's nullify only nullifies if the _owner property is set.
        // We need to ensure we aria-detach from the owner.
        me.updateOwner(null, me.owner);
        ...
        parent?.isContainer && !parent.isDestroying && parent._items?.includes(me) && parent.remove(me);
    ...
    addToOverflowMenu(menu, overrides, onCreateTwin) {
        const overflowTwin = this.ensureOverflowTwin(overrides, onCreateTwin);
    ...
    get twinForwardEventPairs() {
        const value = this._twinForwardEvents;

        return value ? ObjectHelper.entries(value) : EMPTY;
    }
    ...
    configureOverflowTwin(overrides) {
        ...
        // Some obvious properties to not carry forward if present:
        delete config.id;
        ...
        config.owner = me.owner;

        // Ensure we don't have any onFoo for a forwarded 'foo' event:
        me.twinForwardEventPairs.forEach(([ev]) => {
            delete config[`on${StringHelper.capitalize(ev)}`];
        });

        if (twinSyncConfigs) {
            for (key in twinSyncConfigs) {
                val = twinSyncConfigs[key];

                if (val) {
                    // Since we can rename config properties, be sure no properties of 'config' are carried forward from
                    // the source's initialConfig if they have been renamed (e.g., if 'value' is mapped to 'checked',
                    // delete 'config.value' since it may be present in the source's initialConfig)
                    delete config[key];
                    config[typeof val === 'string' ? val : key] = me[key];
                }
            }
        }
    ...
    createOverflowTwin(overrides) {
        ...
        // We've set the twinSyncConfigs in to the config object, so don't perform an initial sync()
        twinSyncConfigs && me.bindConfigs(overflowTwin, twinSyncConfigs, NO_SYNC);

        me.twinForwardEventPairs.forEach(([evFrom, evTo]) => {
            overflowTwin.ion({
                [evFrom] : info => {
                    ...
                    if (me.overflowTwin === info.source) {
                        ...
                        me.trigger(typeof evTo === 'string' ? evTo : info.eventName, info);
                    }
                }
            });
        });
    ...
    attachRef(name, el, domConfig) {
        ...
        // Key elements contain owner pointer if data is supported.
     ...
     */
    compose() {
        const { widgetClassList } = this;

        return {
            class : {
                ...DomClassList.normalize(widgetClassList, 'object')
            }
        };
    }
    ...
    doCompose() {
        ...
        if (hasOwn(me, 'compose')) {
            DomHelper.merge(domConfig, me.compose(domConfig));
        }
    ...
    changeElement(element) {
        ...
        else if (DomHelper.isReactElement(me.peekConfig('html')) || DomHelper.isVueConfig(me.peekConfig('html'))) {
            // Will portal the React/Vue element into the element later
    ...
    updateElement(element) {
        ...
        me._hidden && classes.push('b-hidden', `b-hide-mode-${me.hideMode}`);
        me._readOnly && classes.push('b-readonly');
        classes.push('b-initializing');
        ...
        me.getConfig('role', 'ariaLabel', 'ariaDescription');

        if (me._html) {
            me.getConfig('htmlMutationObserver');
        }

        // If it's an implicit owner (read from this.parent, or acquired from DOM containment)
        // it will not have gone through the updater, so do that now.
        if (me.owner && !me._owner) {
            me.updateOwner(me.owner);
        }

        if (DomHelper.isVueConfig(me._html)) {
            DomHelper.handleVueContent(me, me.html);
        }
    ...
    localizeText(text) {
        return text.match(localizeRE) ? this.L(text, this) : text;
    }
    ...
    updateAriaDescription(ariaDescription) {
        const
            me              = this,
            { ariaElement } = me,
            descElId        = `${me.id}-aria-desc-el`;

        if (ariaDescription) {
            const ariaDescEl = me._ariaDescEl || (me._ariaDescEl = DomHelper.createElement({
                className : 'b-aria-desc-element',
                id        : descElId,
                parent    : ariaElement
            }));

            ariaDescEl.innerText = me.localizeText(ariaDescription);
    ...
    updateAriaLabel(ariaLabel = '') {
        const el = this.labelledElement || this.ariaElement;

        el && DomHelper.setAttributes(el, {
            'aria-label' : ariaLabel ? this.localizeText(ariaLabel) : null
        });
    }

    updateAriaHasPopup(ariaHasPopup) {
        if (ariaHasPopup) {
            this.ariaElement?.setAttribute('aria-haspopup', ariaHasPopup);
        }
        else {
            this.ariaElement?.removeAttribute('aria-haspopup');
        }
    }
    ...
    // For when an owner needs a role other than the default
    get ownersRole() {
        let { role } = this;

        if (role === 'presentation') {
            role = this.query(e => e.role !== 'presentation')?.role;
        }
        return role;
    }
    ...
    getRenderContext(config = this, renderTo) {
        let parentElement                 = renderTo || config.appendTo,
            { insertFirst, insertBefore } = config;
     ...
     */
    get dataset() {
        return this.element?.dataset || EMPTY;
    }
    ...
    get constrainTo() {
        ...
        result = (result === undefined)
            ? globalThis
            : (result?.nodeType === Node.DOCUMENT_FRAGMENT_NODE ? (result.host || result.ownerDocument) : result);
    ...
    updateCentered(value) {
        const
            me                          = this,
            { element, _anchorElement } = me;

        if (value && !me.floating && !me.positioned) {
            throw new Error('`centered` is only relevant when a Widget is `floating` or `positioned`');
        }

        if (value) {
            element.classList.add('b-centered');
            element.style.transform = element.style.left = element.style.insetInlineStart = element.style.right = element.style.top = '';
    ...
    alignTo(spec) {
        ...
        spec = spec.realignTarget ? ObjectHelper.clone(spec) : ObjectHelper.merge({
            aligningToEvent,
            aligningToElement,
            constrainTo,
        ...
        if (typeof spec.offset === 'string') {
            const
                offsetEl   = element.querySelector(spec.offset),
                offsetRect = Rectangle.from(offsetEl),
                offsetPt   = offsetRect.getAlignmentPoint(spec.align),
                outerRect  = Rectangle.from(element),
                outerPt    = outerRect.getAlignmentPoint(spec.align);

            spec.offset = [outerPt[0] - offsetPt[0], outerPt[1] - offsetPt[1]];
        }
        ...
        if (anchor?.edge) {
            ...
            // Anchor and any mask must not be "seen" by childFromPoint
            anchorStyle.display = 'none';
            masked && (masked.element.style.display = 'none');
            ...
            masked && (masked.element.style.display = '');

            if (edge === 'left' || edge === 'right') {
                edge = edge === 'left' ? (me.rtl ? 'end' : 'start') : (me.rtl ? 'start' : 'end');
            }
            ...
            // We are using writing direction independent coordinates.
            svgStyle.left = `${anchor.x || 0}px`;
            svgStyle.translate = `0 ${anchor.y || 0}px`;
        ...
        if ((me.scrollAction === 'realign' && aligningToElement || me.scrollAction === 'hide') && !me.documentScrollListener) {
            ...
            me.setTimeout(() => {
                const
                    el         = aligningToElement ? target : me.lastAlignSpec.anchoredTo,
                    targetRoot = el ? DomHelper.getRootElement(el) : null;
     ...
     * @param {String|HTMLElement} [element] The child element or its name.
    ...
    rectangleOf(element, which, relativeTo, ignorePageScroll) {
        ...
        return Rectangle[which || 'from'](element, relativeTo, ignorePageScroll);
    }
    ...
    releaseSizeConstraints() {
        ...
        // Release any size constraints we may have had due to being aligned close to the
        // end edges of the containing element.
        if (!me.centered) {
            DomHelper.setTopInsetInlineStart(element, 0, 0);
        }
    ...
    toFront() {
        ...
        // If we contain focus and therefore should not be moved, collect all following .b-floating
        // elements and push them into a documentFragment and insert it before this.
        ...
        if (this.containsFocus) {
            for (let followingEl = element.nextSibling, nextEl; followingEl; followingEl = nextEl) {
                ...
                // HTML comment elements do not have a matches method.
                if (parent.contains(followingEl) && followingEl.matches?.('.b-floating,.b-positioned') && !this.owns(followingEl)) {
                    widgetsFrag.appendChild(followingEl);
                }
            }
            widgetsFrag.childElementCount && parent.insertBefore(widgetsFrag, element);
        }
    ...
    changeHtml(html) {
        // React elements and Vue configs should pass through as is, handled in insertContent() later
        if (
            html &&
            typeof html === 'object' &&
            !DomHelper.isVueConfig(html) &&
            !DomHelper.isReactElement(html) &&
            !Array.isArray(html)
        ) {
            html = [html];
        }
    ...
    updateHtml(html) {
        ...
        me.noChildElements = typeof html === 'string' && !htmlRe.test(html);
        if (!me.initializingElement) {
            // When adopting, the html does not replace the initial content
            me.insertContent(html, !(me.initializingHtml && me.adopt));
        }
    }
    ...
    insertContent(newContent, replace = false) {
        ...
        if (DomHelper.isReactElement(newContent)) {
            return DomHelper.handleReactElement(me, newContent);
        }

        if (DomHelper.isVueConfig(newContent)) {
            return DomHelper.handleVueContent(me, newContent);
        }
        ...
        if (newContent) {
            ...
            if (typeof newContent === 'object') {
                ...
                // We're not fully replacing the content, only replacing what was in the contentRange
                contentRange.deleteContents();
                contentRange.insertNode(contentRange.createContextualFragment(newContent));
                done = true;
            }
        ...
        contentElement.classList.toggle('b-no-child-elements', me.noChildElements);
    ...
    onThemeChange() {
        // A theme change may invalidate the anchor element's size or this.defaultAnchorBackgroundColor.
        // Invalidate it, and realign if it is visible.
    ...
    get anchorElement() {
        ...
        if (!me._anchorElement) {
            const
                useFilter = me.floating || me.positioned,
    ...
    updateAnchor(anchor) {
        this._anchorElement?.classList.toggle('b-hide-display', !anchor);
    }

    changeDraggable(draggable, was) {
        const me = this;

        was?.destroy?.();
        if (draggable) {
            const config = typeof draggable === 'object' ?  draggable : {};

            me.draggableHandleSelector = config.handleSelector;
            delete config.handleSelector;

            draggable = new DragHelper(ObjectHelper.merge({
                name              : 'widget',
                target            : me,
                floatRootOwner    : me,
                rtlSource         : me,
                positioning       : me.floating ? 'inset' : '',
                dragWithin        : me.floating ? undefined : me.element.parentNode,
                internalListeners : {
    ...
    onBeforeDragStart() {
        const me = this;
        if (me.validateDragStartEvent(me.draggable.startEvent)) {
            const { x, y, element } = me;
            ...
            if (me.centered) {
                DomHelper.setTopInsetInlineStart(element, y, x);
            }
            return true;
        }
        return false;
    ...
    validateDragStartEvent(e) {
        const
            ...
            { draggableHandleSelector, draggableBlacklistSelector } = me;

        if (draggableBlacklistSelector && actualTarget.closest(draggableBlacklistSelector)) {
            return false;
        }

        if (draggableHandleSelector) {
            const blacklist = negationPseudo.exec(draggableHandleSelector)?.[1]; // Extract the content of :not()

            // If the selector was :not(), then if we are a descendant of a matching element, it's a no-drag
            if (blacklist && actualTarget.closest(blacklist)) {
                return false;
            }
            // If we are not the descendant of a matching element, it's a no-drag
            else if (!actualTarget.closest(`${draggableHandleSelector}`)) {
                return false;
            }
        }
    ...
    updatePositionable(positionable) {
        this.element.classList.toggle('b-positionable', Boolean(positionable));

        if (positionable) {
            this.innerItem = false;
        }
    }
    ...
    updatePositioned(positioned) {
        ...
        element.classList.toggle('b-positioned', Boolean(positioned));

        if (positioned) {
            this.innerItem = false;
        }

        element[positioned ? 'setAttribute' : 'removeAttribute']('tabindex', positioned ? '-1' : undefined);
    }

    getXY() {
        const { element } = this;

        return [DomHelper.getPageX(element), DomHelper.getPageY(element)];
    }
    ...
    setXY(x, y) {
        ...
        if (me.floating || me.positioned) {
            ...
            if (DomHelper.getStyleValue(element, 'position') === 'fixed') {
                ...
                DomHelper.setTopInsetInlineStart(element, y, element.offsetParent.offsetWidth - element.offsetWidth - x - 1);
            }
            else {
                DomHelper.setTopInsetInlineStart(element, y, x);
            }
    ...
    setPos(name, value) {
        ...
        [name, value] = DomHelper.unitize(name, value ?? '');

        me.element.style[name] = value;

        if (me.top == null && me.right == null && me.bottom == null && me.left == null) {
            if (positionable === 'after' || positionable === 'before') {
                me.positionable = null;
            }
        }
        else if (!positionable) {
            me.positionable = 'before';
        }
    }

    updateBottom(bottom) {
        this.setPos('bottom', bottom);
    }

    updateLeft(left) {
        this.setPos('inset-inline-start', left);
    }

    updateRight(right) {
        this.setPos('inset-inline-end', right);
    }
    ...
    updateDisabled(disabled = false, was) {
        const
            me                                     = this,
            { element, focusElement, ariaElement } = me;
        ...
        if (disabled) {
            ...
            // If some focus listener changed our state, we must not continue (booleanize to ignore 'inert')
            if (!me._disabled !== !disabled) {
                return;
            }
        }
        ...
        if (element) {
            ...
            // This disabled tabbing into as well as clicking on the widget.
            if (disabled === 'inert') {
                element.inert = true;
            }
            else if (was === 'inert') {
            ...
            if (ariaElement) {
                ariaElement.setAttribute('aria-disabled', Boolean(disabled));
            }
        }
    ...
    changeTooltip(tooltip, oldTooltip) {
        ...
        if (tooltip) {
            if (!(me.preventTooltipOnTouch && BrowserHelper.isTouchDevice)) {
                ...
                const needNewInstance = oldTooltip?.isTooltip || tooltip.newInstance || tooltip.items || tooltip.bbar || tooltip.tbar;
                ...
                if (needNewInstance) {
                    ...
                    result = tooltip = Widget.reconfigure(oldTooltip, tooltip, me);
        ...
        if (tooltip && !me.ariaLabel && me.tooltipText) {
            element.setAttribute('aria-label', me.localizeText(me.tooltipText));
        }
        return result;
    }
    ...
    get isVisible() {
        ...
        // Do not trigger recompose by accessing the element property if we are hidden.
        return Boolean(!me._hidden && (element = this.element) && !me.isDestroying && element.isConnected &&
            (!me.requireSize || hasLayout(element)) &&
            (!me.parent || me.parent.isVisible)
        );
    }

    get isVisibleForValidation() {
        const
            me          = this,
            { element } = me;
        ...
        // If we are hidden, or destroying, or any ancestors are hidden, we're not visible
        return Boolean(
            element && (!me._hidden || (me.tab && !me.tab.hidden)) && !me.isDestroying && element.isConnected &&
            //(!me.requireSize || hasLayout(element)) &&
            (!(owner = me.containingWidget) || owner.isVisibleForValidation)
        );
    }
     ...
     */
    focus() {
        this.focusableElement?.focus({ preventScroll : true });
    }

    /**
     * Returns an object describing the focus and keyboard navigation aspects of this widget's {@link #property-focusElement}.
     * @property {Focusability}
     * @readonly
     * @category DOM
     * @internal
     */
    get focusability() {
        return DomHelper.getFocusability(this.focusElement);
    }

    /**
     * Returns this widget's {@link #property-focusElement} if that element can currently be given focus (e.g., this
     * widget is not disabled, or hidden).
     * @property {HTMLElement}
     * @readonly
     * @category DOM
     * @advanced
    ...
    get focusableElement() {
        ...
        const
            me           = this,
            focusElement = (!me.isDestroying && me.isVisible && !me.disabled) && me.focusElement;
        ...
        return (focusElement === document.body || focusElement?.offsetParent) && focusElement;
    }
    ...
    get isFocusable() {
        return Boolean(this.focusableElement);
    }

    get isTabbable() {
        return this.focusability.tabbable;
    }

    set isTabbable(value) {
        const
            { focusability, focusElement } = this,
            [add, rem]                     = focusability.natural ? [!value && '-1', value] : [value && '0', !value];

        if (focusElement && !value !== !focusability.tabbable) {  // booleanize
            rem && focusElement.removeAttribute('tabIndex');
            add && focusElement.setAttribute('tabIndex', add);
        }
    }
    ...
    async show({ align, animate = true } = {}) {
        ...
        if (trigger !== false && me.internalBeforeShow?.() !== false) {
            return new Promise(resolve => {
                ...
                // This need to be here, because it breaks animation otherwise
    ...
    onChildHide(hidden) {
        ...
        if (hidden.floating && ariaElement) {
            // We were described by the outgoing if it was a tooltip
            if (ownersRole === 'tooltip') {
                const
                    { titleElement } = hidden,
                    titleId          = titleElement && DomHelper.getId(titleElement),
                    id               = DomHelper.getId(hidden.contentElement);

                if (titleId) {
                    DomHelper.removeAttributeValue(ariaElement, 'aria-describedby', titleId);
                }
                DomHelper.removeAttributeValue(ariaElement, 'aria-describedby', id);
            }
            // Else, we controlled it
            else {
                this.ariaHasPopup = null;
                DomHelper.removeAttributeValue(ariaElement, 'aria-controls', DomHelper.getId(hidden.ariaElement || hidden.element));
            }
        }
    }
    ...
    onChildShow(shown) {
        ...
        if (shown.floating && ariaElement) {
            // We are described by the shown widget if it is a tooltip
            if (ownersRole === 'tooltip') {
                const
                    { titleElement } = shown,
                    titleId          = titleElement && DomHelper.getId(titleElement),
                    id               = DomHelper.getId(shown.contentElement);

                if (titleId) {
                    DomHelper.addAttributeValue(ariaElement, 'aria-describedby', titleId);
                }
                DomHelper.addAttributeValue(ariaElement, 'aria-describedby', id);
            }
            // Else, we control it
            else {
                this.ariaHasPopup = ownersRole;
                DomHelper.addAttributeValue(ariaElement, 'aria-controls', DomHelper.getId(shown.ariaElement || shown.element));
            }
        }
    }
    ...
    triggerPaint() {
        ...
        if (me.isVisible) {
            if (firstPaint) {
                me.getConfig('scrollable', me.floating || me.positioned ? 'draggable' : null);
            ...
            me.afterInternalPaint?.({ firstPaint });
    ...
    hide(animate = true) {
        return new Promise(resolve => {
            ...
            if (!me._hidden && me.trigger('beforeHide', { animate }) !== false) {
                ...
                // Hide floating widgets owned by us
                me.ownedWidgets && Object.values(me.ownedWidgets).forEach(w => w.floating && w.hide?.(animate));
    ...
    doHideOrRealign({ target, type, isTrusted }) {
        const
            me         = this,
            {
                lastAlignSpec,
        ...
        if (
            ...
            target && (
                // If a scrolled element is not visible, ignore it.
                (type === 'scroll' && !DomHelper.isVisible(target)) ||
                ...
                (
                    ...
                    element.contains(activeEl) && DomHelper.isTextualInput(activeEl) &&
        ...
        // Perform the realignment. Code may need to know if it's due to scrolling.
        me.scrollRealigning = type === 'scroll';
        me.realign();
        me.scrollRealigning = false;
    ...
    afterHideAnimation() {
        ...
        if (!me.hasOwnProperty('_hideMode') && me.floating && me.floatRoot.contains(element)) {
            element.remove();
        }
        else {
            me.syncHiddenCls();
    ...
    syncHiddenCls(_hidden = this._hidden) {
        const
            { element, hideMode } = this,
            classes               = {
                'b-hidden' : _hidden
            };

        for (const c of hideModes) {
            // Once we add b-hide-mode-opacity/clip, do not remove it (in case we want to use transitions. No point
            // doing this for display and we keep the DOM cleaner by removing it (as the default it would affect most
            // widgets)
            classes[`b-hide-mode-${c}`] = c === hideMode && (_hidden || c !== 'display');
        }

        DomHelper.assignClasses(element, classes);
    }
    ...
    /**
     * The owning Widget of this Widget. If this Widget is directly contained (i.e. it is one
     * of the {@link Core.widget.Container#property-items} of a Container), __this property will
     * be read-only, and will refer to the containing Widget__.
     *
     ...
     * is destroyed.
     * @member {Core.widget.Widget|Grid.column.Column}
     * @typings {ContainerItem|GridColumn}
     * @readonly
     * @category Widget hierarchy
     */
    get owner() {
    ...
    updateOwner(owner, previousOwner) {
        ...
        if (owner && owner !== me.parent) {
            ((owner.ownedWidgets || (owner.ownedWidgets = {}))[id] = me);
        }
    }
     ...
     * @category Widget hierarchy
     */
    owns(target, includeOwnedWidgets) {
        if (target) {
            ...
            if (target.nodeType === Element.ELEMENT_NODE) {
                ...
                target = target.getAttribute('owned-by') ? Widget.getById(target.getAttribute('owned-by')) : Widget.fromElement(target);
            }



            while (target) {
                if (target.isHint || target === this || (includeOwnedWidgets && this.ownedWidgets && Object.values(this.ownedWidgets).some(ownedWidget => ownedWidget.owns(target, true)))) {
                    return true;
                }
                target = target.owner;
            }
    ...
    changeMaximizeOnMobile(maximizeOnMobile) {
        ...
        if (me.floating && BrowserHelper.isMobile) {
            ...
            if (maximizeOnMobile) {
                ...
                me.centered = initialConfig.centered;
                me.modal = initialConfig.modal;
    ...
    updateHideMode() {
        this.hasPainted && this.syncHiddenCls();
    }
    ...
    get floatRoot() {
        ...
        floatRoot.classList.toggle('b-rtl', Boolean(me.rtl));
     ...
     */
    get focusVisible() {
        return DomHelper.getActiveElement(this.element).matches(':focus-visible');
    }
    ...
    static get focusVisible() {
        return DomHelper.getActiveElement().matches(':focus-visible');
    }
    ...
    // Convenience fn to replace WidgetHelper.append in tests
    static append(items, config = {}) {
        ...
        return ArrayHelper.asArray(items).map(item => Widget.create(ObjectHelper.assign({}, config, item)));
    }
    ...
    updateRtl(rtl) {
        ...
        // Cascade the rtl setting to owned floaters which are not configured with an rtl value
        this.ownedWidgets && Object.values(this.ownedWidgets).forEach(widget => {
            if (!('rtl' in widget.initialConfig)) {
                widget.rtl = rtl;
            }
        });
    }

    // If implementer did not specify a scrollAction, realign anchored floaters by default
    get scrollAction() {
        let { _scrollAction } = this;

        if (!_scrollAction && this.anchor) {
            _scrollAction = 'realign';
        }
    ...
    announceAriaLive(message) {
        if (this.element?.isConnected) {
            if (!this.ariaLiveEl) {
                this.ariaLiveEl = DomHelper.createElement({
                    parent      : this.element,
                    class       : 'b-arialive-el',
                    'aria-live' : 'polite'
                });
            ...
            this.ariaLiveEl.textContent = message?.text || message;
        }
    }
}
...
Object.assign((globalThis.bryntum || (globalThis.bryntum = {})), {
    Widget,
    // globalThis.$0 is the currently selected element in Chrome DevTools
    get          : (id = globalThis.$0) => id.nodeType ? Widget.fromElement(id) : Widget.getById(id),
    query        : Widget.query,
