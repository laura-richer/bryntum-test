...
import { isGeneratorFunction } from "../../../../ChronoGraph/util/Helpers.js";
...
__decorate([
    field()
], BaseAllocationInfo.prototype, "respectStoreFilters", void 0);
__decorate([
    field()
], BaseAllocationInfo.prototype, "assignmentFilterFn", void 0);
__decorate([
    field()
...
export class ResourceAllocationInfo extends BaseAllocationInfo {
    ...
    leaveGraph(graph) {
        const { resource } = this;
        if (resource && this.eventRangesCalendar) {
            resource.getProject().clearCombinationsWith(this.eventRangesCalendar);
        }
    ...
    *shouldIncludeAssignmentInAllocation(assignment) {
        const event = yield assignment.$.event, includeInactiveEvents = yield this.$.includeInactiveEvents, respectStoreFilters = yield this.$.respectStoreFilters, assignmentFilterFn = yield this.$.assignmentFilterFn;
        let result = Boolean(event);
        // if the assignment got event
        if (event) {
            // if we should treat only active events
            if (!includeInactiveEvents) {
                result = result && !(yield event.$.inactive);
            }
            // if we should take into account filters
            if (respectStoreFilters) {
                const project = this.resource.getProject();
                const eventStore = project.getEventStore();
                const assignmentStore = project.getAssignmentStore();
                // Subscribe to the stores filter calls
                yield* eventStore.trackStoreFilter();
                yield* assignmentStore.trackStoreFilter();
                result = result
                    && eventStore.isAvailable(event)
                    && assignmentStore.isAvailable(assignment);
            }
            // if a filtering function is provided
            if (assignmentFilterFn) {
                result = result
                    && (isGeneratorFunction(assignmentFilterFn) ? (yield* assignmentFilterFn(assignment, this))
                        : assignmentFilterFn(assignment, this));
            }
        }
        return result;
    }
    /**
     * Returns the value of the provided field of the provided record
     * and subscribes to future field changes by setting up a special proxy identifier
     * representing the field on the graph and yielding it.
     * @param record Record to read value from
     * @param fieldName Field to get value of
     * @returns The field value
     */
    *readField(record, fieldName) {
        // Yield if that's an Entity field
        if (record.$[fieldName]) {
            return yield record.$[fieldName];
        }
        // If that's a regular Model field
        const store = record.firstStore;
        // make a special identifier to track the field changes
        if (store instanceof ChronoPartOfProjectStoreMixin) {
            const identifier = store.trackRecordFieldChanges(record, fieldName);
            // and yield it
    ...
    *calculateAllocation() {
        ...
        const maxUnits = yield resource.$.maxUnits;
        ...
        for (const assignment of assignments) {
            ...
            const ignoreResourceCalendar = yield event.$.ignoreResourceCalendar ||
                ...
                (yield* event.hasElapsedDuration());
        ...
        for (const tick of ticksCalendar.intervalStore) {
        ...
        yield* resource.forEachAvailabilityInterval(iterationOptions, (intervalStartDate, intervalEndDate, intervalData) => {
            ...
            if (isWorkingCalendar.get(ticksCalendar)) {
                ...
                if (isWorkingCalendar.get(calendar)) {
                    tickData.maxEffort += intervalDuration * maxUnits / 100;
                }
                ...
                if (intervalHasAssignments) {
                    ...
                    tickData.isOverallocated = tickData.isOverallocated
                        || tickData.effort > tickData.maxEffort
                        || tickData.units > maxUnits;
                    tickData.isUnderallocated = tickData.isUnderallocated
                        || intervalEffort < intervalDuration
                        || tickData.units < maxUnits;
                }
            }
        });
        if (lastTick) {
            const lastTicksData = ticksData.get(lastTick);
            const maxUnits = lastTicksData.resource.maxUnits;
            // last attempt to detect underallocation in the tick - check if its collected effort is less than its maxEffort
            lastTicksData.isUnderallocated = lastTicksData.isUnderallocated
                || (lastTicksData.effort && lastTicksData.effort < lastTicksData.maxEffort);
        }
        return {
            owner: this,
            resource,
...
export class SchedulerProResourceMixin extends Mixin([BaseResourceMixin], (base) => {
    ...
    __decorate([
        model_field({ type: 'number', defaultValue: 100 }, { sync: true })
    ], SchedulerProResourceMixin.prototype, "maxUnits", void 0);
