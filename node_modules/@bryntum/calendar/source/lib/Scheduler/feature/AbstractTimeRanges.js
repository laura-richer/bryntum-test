...
export default class AbstractTimeRanges extends InstancePlugin.mixin(Delayable) {
        ...
        if (!client.hideHeaders) {
            ...
            if (me.enableResizing) {
                me.drag = DragHelper.new({
                    ...
                    constrain          : true,
                    outerElement       : me.headerContainerElement.parentElement,
     ...
     * @param {Date} [viewStartDate] Specifies view start date. Defaults to view visible range start
     * @param {Date} [viewEndDate] Specifies view end date. Defaults to view visible range end
     * @returns {Boolean} `true` if the range should be rendered and `false` otherwise.
     */
    shouldRenderRange(
        range,
        viewStartDate = this.client.visibleDateRange.startDate,
        viewEndDate   = this.client.visibleDateRange.endDate
    ) {
        const
    ...
    getRangeDomConfig(timeRange, minDate, maxDate, relativeTo = 0) {
        ...
        let { startDate : timeRangeStart, endDate : timeRangeEnd } = timeRange;

        // Handle fillTicks, stretch the time range
        if (client.fillTicks && (timeRangeEnd && timeRangeEnd - timeRangeStart !== 0 || timeRange.fillAlign)) {
            timeRangeStart = timeRangeEnd || timeRange.fillAlign === 'start' ? timeAxis.getSnappedTickFromDate(timeRangeStart).startDate : timeAxis.getSnappedTickFromDate(timeRangeStart).endDate;
            if (timeRangeEnd) {
                const
                    endTickIndex = Math.ceil(timeAxis.getTickFromDate(timeRangeEnd)) - 1,
                    endTick      = timeAxis.getAt(endTickIndex);
                timeRangeEnd = endTick.endDate;
            }
        }

        const
            startPos   = client.getVirtualCoordFromDate(DateHelper.max(timeRangeStart, minDate), {
                respectExclusion : true
            }) - relativeTo,
            endPos     = timeRangeEnd ? client.getVirtualCoordFromDate(DateHelper.min(timeRangeEnd, maxDate), {
                respectExclusion : true,
                isEnd            : true
            }) - relativeTo : startPos,
        ...
        return {
            ...
            style : client.isVertical
                ? `inset-block-start: ${startPos}px; ${isRange ? `height:${size}px` : ''};`
                : `inset-inline-start: ${startPos}px; ${isRange ? `width:${size}px` : ''};`
        };
    }
    ...
    renderRange(timeRange, startDate, endDate) {
        ...
        if (client.timeAxisViewModel.availableSpace > 0 && timeAxis.startDate && me.shouldRenderRange(timeRange, startDate, endDate)) {
            const
    ...
    get headerContainerElement() {
        const
            me                             = this,
            { isVertical, timeAxisColumn } = me.client;

        if (!me._headerContainerElement) {
            // Render into the subGrid´s header element or the vertical timeaxis depending on mode
            if (isVertical) {
                // Add element to the subgrid element to keep it intact between row refreshes
                me._headerContainerElement = me.client.subGrids.locked.element;
            }
            else if (!isVertical) {
    ...
    onTimeRangeClick(event) {
        ...
        this.client.trigger(`timeRangeHeader${StringHelper.capitalize(event.type)}`, { domEvent : event, timeRangeRecord });
    }
    ...
    showTip(context) {
        ...
        if (me.showTooltip) {
            ...
            me.tip = Tooltip.reconfigure(me.tip, Tooltip.mergeConfigs({
                type                     : 'tooltip',
