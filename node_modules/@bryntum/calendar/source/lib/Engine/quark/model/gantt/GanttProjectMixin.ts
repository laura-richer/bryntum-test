...
import { ConstraintType, Direction, EffectiveDirection, ProjectType } from '../../../scheduling/Types.js'
...
import { HasDateConstraintMixin } from "../scheduler_pro/HasDateConstraintMixin.js"
...
import { GanttProjectScheduleMixin } from "./GanttProjectScheduleMixin.js"
...
export class GanttProjectMixin extends Mixin(
    ...
    class GanttProjectMixin extends base {
        ...
        static $name = 'GanttProjectMixin'

        // @ts-ignore
        static scheduleMixinClass  : typeof GanttProjectScheduleMixin  =
            Mixin(
                [ base.scheduleMixinClass, GanttProjectScheduleMixin ],
                base => base
            )
        ...
        /**
         * Class to represent [[ProjectConstraintInterval|project constraint]] interval
         * applied by the project to its events.
         * The interval prevents the events to be moved before the project start (for forward scheduled projects)
        ...
        manuallyScheduledParentBreaksConstraintInheritance                : boolean

        /**
         * **This option is experimental and subject for possible change**.
         *
         * The second scheduling pass is a backward pass ("late" start/end dates) for forward-scheduled project and forward pass
         * ("early" start/end dates) for backward-scheduled project.
         *
         * When this flag is enabled, if a conflict between the task's constraint and incoming dependencies occurs
         * during the 2nd scheduling pass, the task's constraint is ignored and task is scheduled according to its dependencies.
         *
         * This happens only during the specified type of conflict. For other types of conflict and for no-conflict case
         * scheduling is performed as usual.
         *
        ...
        * calculateNonEqual () : CalculationIterator<boolean> {
        ...
        construct (config : Partial<this> = {}) {
            ...
            this.manuallyScheduledParentBreaksConstraintInheritance = config.manuallyScheduledParentBreaksConstraintInheritance ?? true
        }
        ...
        * unsafeCalculateInitialMinChildrenStartDateDeep () : CalculationIterator<Date> {
            const childEvents : Set<HasDateConstraintMixin>    = yield this.$.childEvents

            // note, that we do not yield here, as we want to calculate "initial" project start date
            ...
            let result : Date       = MAX_DATE,
                child : HasDateConstraintMixin

            const toProcess : HasDateConstraintMixin[]        = [...childEvents]

            while ((child = toProcess.shift())) {
                let childDate     = yield UnsafeProposedOrPreviousValueOf(child.$.startDate)

                // in case a task has no start date but has constraint provided - use that value
                if (!childDate) {
                    const childConstraintType : ConstraintType  = yield UnsafeProposedOrPreviousValueOf(child.$.constraintType)
                    const childConstraintDate : Date            = yield UnsafeProposedOrPreviousValueOf(child.$.constraintDate)

                    if (
                        (childConstraintType === ConstraintType.MustStartOn || childConstraintType === ConstraintType.StartNoEarlierThan)
                        && childConstraintDate
                    ) {
                        childDate = childConstraintDate
                    }
                }
        ...
        * unsafeCalculateInitialMaxChildrenEndDateDeep () : CalculationIterator<Date> {
            ...
            while ((child = toProcess.shift())) {
                ...
                if (!childDate) {
                    ...
                    if (
                        (childConstraintType === ConstraintType.MustFinishOn || childConstraintType === ConstraintType.FinishNoLaterThan)
                        && childConstraintDate
                    ) {
                        childDate = childConstraintDate
                    }
        ...
        getDependencyCycleDetectionIdentifiers (
            fromEvent : HasDependenciesMixin & ConstrainedLateEventMixin,
            toEvent : HasDependenciesMixin & ConstrainedLateEventMixin
        )
            : FieldIdentifier[]
        {
            return [
                ...
                toEvent.lateSchedule.$.endDateConstraintIntervals,
                toEvent.lateSchedule.$.startDateConstraintIntervals
            ]
        }
        ...
        * createStartDateConstraintsInterval (
        ...
        {
            // stub this method, project does not create intervals for its constraints
            // instead it creates special ProjectConstraintInterval intervals directly from its start/end dates
        }
...
export class ProjectConstraintIntervalDescription extends ConstraintIntervalDescription {
    ...
    static $name = 'ProjectConstraintIntervalDescription'

    static getDescription (interval : ProjectConstraintInterval) : string {
        interval = interval.reflectionOf as ProjectConstraintInterval || interval

        return format(interval.startDateIsFinite() ? this.L('L{startDateDescriptionTpl}') : this.L('L{endDateDescriptionTpl}'), ...this.getDescriptionParameters(interval))
    }
