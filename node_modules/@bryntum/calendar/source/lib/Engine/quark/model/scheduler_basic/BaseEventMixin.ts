...
export class BaseEventMixin extends Mixin(
    ...
    class BaseEventMixin extends base {
         ...
         */
        @model_field({ type : 'date', bypassEqualityOnSyncDataset : true }, { converter : dateConverter })
        startDate       : Date
         ...
         */
        @model_field({ type : 'number', bypassEqualityOnSyncDataset : true, allowNull : true })
        duration        : Duration

        /**
         * The duration unit of the event's duration. See also [[duration]].
         */
        @model_field({ type : 'durationunit', defaultValue : TimeUnit.Day }, { converter : (unit : string) => DateHelper.normalizeUnit(unit) || TimeUnit.Day })
        ...
        * isManuallyScheduled () : CalculationIterator<boolean> {
            return yield this.$.manuallyScheduled
        }

        * hasElapsedDuration () : CalculationIterator<boolean> {
            const durationUnit : TimeUnit = yield this.$.durationUnit

            return this.getProject().isElapsedDurationUnit(durationUnit)
        }
        ...
        * calculateEffectiveCalendar () : CalculationIterator<BaseCalendarMixin> {
            // For tasks with elapsed duration we use default 365 days 24/7 calendar
            if ((yield * this.hasElapsedDuration())) {
                return this.getProject().defaultCalendar as BaseCalendarMixin
            }

            return yield * super.calculateEffectiveCalendar()
        }
        ...
        * skipNonWorkingTime (date : Date, isForward : boolean) : CalculationIterator<Date> {
        ...
        /**
         * Returns the [[DurationConverterMixin]] instance used by the event to convert duration units.
         * By default the project instance is used. Override this to use another [[DurationConverterMixin]] instance
         * for duration units conversion performed by the event.
         */
        getDurationConverter() : DurationConverterMixin {
            return this.getProject()
        }


        /**
         * Indicates whether the event can perform the provided duration conversion.
         * The event might not be able to convert units on early stages of data loading when
         * the project has not loaded its conversion rates yet.
         * So the method by default checks that the project has loaded the rates
         * needed for conversion.
         * @param duration Duration value
         * @param fromUnit Duration value time unit
         * @param toUnit   Target time unit to convert the value to
         */
        canConvertDuration (duration : Duration, fromUnit : TimeUnit, toUnit : TimeUnit) : boolean {
            const converter = this.getDurationConverter()

            toUnit   = DateHelper.normalizeUnit(toUnit) as TimeUnit
            fromUnit = DateHelper.normalizeUnit(fromUnit) as TimeUnit

            const unitsInMs = converter?.unitsInMs

            return Boolean(typeof duration === 'number' && unitsInMs?.[ fromUnit ] && unitsInMs[ toUnit ])
        }
         ...
         * @param toUnit   Target time unit to convert the value to
         */
        * convertDurationGen (duration : Duration, fromUnit : TimeUnit, toUnit : TimeUnit) : CalculationIterator<Duration> {
            const converter = this.getDurationConverter()

            if (converter === (this as any as DurationConverterMixin)) {
                // @ts-ignore
                return yield* super.$convertDuration(duration, fromUnit, toUnit)
            }
        ...
        * skipWorkingTime (date : Date, isForward : boolean, duration : Duration, unit? : TimeUnit) : CalculationIterator<Date> {
            ...
            if (unit && unit !== durationUnit) {
                duration    = yield* this.convertDurationGen(duration, unit, durationUnit)
            }
        ...
        writeStartDate (me : Identifier, transaction : Transaction, quark : Quark, date : Date, keepDuration : boolean = true) {
            ...
            if (!this.getProject().isStmRestoring) {
                ...
                transaction.write(this.$.unscheduled, date == null)
            }
        ...
        * calculateProjectedXDateWithDuration (baseDate : Date, isForward : boolean, duration : Duration) : CalculationIterator<Date> {
            ...
            if (isForward) {
                return calendar.calculateEndDate(baseDate, yield* this.convertDurationGen(duration, durationUnit, TimeUnit.Millisecond))
            } else {
                return calendar.calculateStartDate(baseDate, yield* this.convertDurationGen(duration, durationUnit, TimeUnit.Millisecond))
            }
        }
        //endregion
        ...
        * calculateEndDateProposed () : CalculationIterator<Date> {
        ...
        getDuration (unit? : TimeUnit) : Duration {
            ...
            return unit !== undefined ? this.run('convertDurationGen', duration, this.durationUnit, unit) : duration
        }
        ...
        writeDuration (me : Identifier, transaction : Transaction, quark : Quark, duration : Duration, unit? : TimeUnit, keepStart : boolean | Instruction = undefined) {
            ...
            if (!this.getProject().isStmRestoring) {
                // this is basically: this.unscheduled = date == null, however it will work with branches
                transaction.write(this.$.unscheduled, duration == null)
            }
        ...
        * calculateProjectedDuration (
            startDate : Date, endDate : Date, durationUnit? : TimeUnit, _iteratorOptions?, _allowNegative : boolean = false
        )
            : CalculationIterator<Duration>
        {
            if (!startDate || !endDate) return null
            ...
            return yield* this.convertDurationGen(calendar.calculateDurationMs(startDate, endDate), TimeUnit.Millisecond, durationUnit)
        }
