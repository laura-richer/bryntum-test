 ...
 *   where the context menu was triggered. Set {@link Grid/feature/RowCopyPaste#config-rowOptionsOnCellContextMenu} to
 *   `true` to show two sets of options when context menu is triggered on a selected cell.
...
export default class CellCopyPaste extends CopyPasteBase {
    ...
    static configurable = {
        ...
        // Disables non-string data copying
        allowedDataSources : 'none'
    };
    ...
    isActionAvailable({ actionName }) {
        return super.isActionAvailable(...arguments) && this.canCopy && (actionName === 'copy' || this.canCutPaste);
    }
    ...
    async copy(isCut = false) {
        ...
        if (cells) {
            ...
            if (await me.writeToClipboard(copiedDataString, isCut, { cells }) === undefined) {
                return;
            }
            ...
            if (isCut === true) {
                for (const cell of cells) {
                    ...
                    if (!column.readOnly) {
                        ...
                        // If the column provides a clipboard string parser, use that to get a value which works as an
                        // empty value for this field.
                        if (column.fromClipboardString) {
                            string = column.fromClipboardString({ string, record });
                        }

                        cell.record.set(cell.column.field, string);
                    }
                }
            }
             ...
             * @param {Grid.util.GridLocation[]} cells The cells about to be copied or cut
    ...
    async paste() {
        ...
        const clipboardData = (await me.readFromClipboard({}, true))?.data;
         ...
         * @param {Grid.util.GridLocation} targetCell The cell from which the paste will be started
    ...
    async beforePaste({ data, text }) {
        ...
        return await this.client.trigger('beforePaste', {
            clipboardData : data, targetCell : this.client.selectedCell, entityName : this.entityName, text
        });
    }
