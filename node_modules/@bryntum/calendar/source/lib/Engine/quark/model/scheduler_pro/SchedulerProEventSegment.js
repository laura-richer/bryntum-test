...
export class SchedulerProEventSegment extends Mixin([BaseEventMixin], (base) => {
    class SchedulerProEventSegment extends base {
        ...
        *calculateEndOffsetByMasterDurationAndStartOffset() {
            ...
            let result;
            let masterDurationMs = yield* this.convertDurationGen(masterDuration, masterDurationUnit, TimeUnit.Millisecond);
            const segments = [];
            let segment = this;
        ...
        *calculateManuallyScheduled() {
        ...
        afterChange(toSet, wasSet, silent, fromRelationUpdate, skipAccessors) {
            ...
            if (wasSet && event) {
                ...
                event.afterChange({ segments: newSegmentsValue }, { segments: newSegmentsValue }, 
                // this flag is forced to `true` in the `ChronoPartOfProjectStoreMixin.ts`
                // we work around with `applyingSyncResponse`
                silent, fromRelationUpdate, skipAccessors);
