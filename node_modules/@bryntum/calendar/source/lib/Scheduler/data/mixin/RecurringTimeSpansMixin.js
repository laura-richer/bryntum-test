...
export default Target => class RecurringTimeSpansMixin extends (Target || Base) {
    static $name = 'RecurringTimeSpansMixin';
    ...
    static configurable = {
         ...
         *
         * By default, changing the {@link Scheduler.model.TimeSpan#field-startDate}
         * to a date that is *not* visited by the recurrence pattern does not recalculate that
         * pattern. It merely changes the date at which that pattern begins.
         *
         * So if an event is explicitly configured to repeat every Monday (The
         * {@link Scheduler.model.mixin.RecurringTimeSpan#field-recurrenceRule} contains `BYDAY=MO`)
         * and the base event is changed by any means to have its {@link Scheduler.model.TimeSpan#field-startDate}
         * on the following Tuesday, visually, the effect would be that the base event "disappears" and
         * the following occurrences *do not move*.
         *
         * The recurrence pattern still repeats every Monday. It's just that that pattern is
         * calculated starting from a different date.
         *
         * To have a rescheduling change to a recurring event *base* recalculate the recurrence
         * pattern to conform to the new start date - to include that date in the pattern - configure
         * `autoAdjustRecurrence` as `true`
         * @prp {Boolean}
         * @default false
         */
        autoAdjustRecurrence : null
    };
