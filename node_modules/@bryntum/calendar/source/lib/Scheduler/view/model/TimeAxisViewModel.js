...
export default class TimeAxisViewModel extends Events() {
    ...
    update(availableSpace, silent = false, forceUpdate = false) {
        ...
        const oldWeekStartDay = DH.weekStartDay;
        if (typeof me.owner?.weekStartDay === 'number') {
            // Slightly hackish, but no other way to format based on settings in the Scheduler/Gantt. So we temporarily
            // adjust the week start day to the one set in the owner
            DH._weekStartDay = timeAxis.weekStartDay;
        }
        ...
        if (me.owner) {
            DH._weekStartDay = oldWeekStartDay;
        }
            ...
            if (include.day && DH.compareUnits(unit, 'day') > 0) {
                ...
                if (checkDay < from || checkDay >= to) {
                    ...
                    if (snapToNextIncluded) {
                        ...
                        date = DH.startOf(date, 'd', false, timeAxis.weekStartDay);
    ...
    set availableSpace(space) {
        const
            me                = this,
            oldAvailableSpace = me._availableSpace;
        ...
        if (me._availableSpace > 0) {
            ...
            if (newTickSize > 0 && (newTickSize !== me.tickSize || oldAvailableSpace === 0)) {
                me.update();
            }
        }
    }
    ...
    getDates(level = this.columnLinesFor, useLowestHeader = false, getEnd = false) {
        const
            ...
            hasGenerator  = Boolean(me.headers && me.headers[linesForLevel].cellGenerator);
        ...
        const
            ...
            createCellContext             = (start, end, i, isLast, data) => {
                ...
                if (headerRowConfig.renderer) {
                    value = headerRowConfig.renderer.call(headerRowConfig.thisObj || me, start, end, cellData, i, me.owner);
