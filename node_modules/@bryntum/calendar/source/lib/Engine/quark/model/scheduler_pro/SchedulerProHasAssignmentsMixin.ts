...
import { BaseCalendarMixin, BaseEmptyCalendarEffectResolution, EmptyCalendarEffect } from "../scheduler_basic/BaseCalendarMixin.js"
...
import { EffectResolutionResult, SchedulingIssueEffectResolution } from "../../../chrono/SchedulingIssueEffect.js"
    ...
    class SchedulerProHasAssignmentsMixin extends base {
        ...
        * forEachAvailabilityInterval (
        ...
        ) : CalculationIterator<CalendarIteratorResult>
        {
            ...
            const effectiveCalendarsCombination : CalendarCacheMultiple     = yield this.$.effectiveCalendarsCombination

            const ignoreResourceCalendar : boolean = (yield this.$.ignoreResourceCalendar) ||
                // For tasks with elapsed duration we don't want resource calendars to rule
                (yield * this.hasElapsedDuration()) ||
                options.ignoreResourceCalendar || !assignmentsByCalendar.size
        ...
        * calculateAssignmentsByCalendar  () : CalculationIterator<this[ 'assignmentsByCalendar' ]> {
            ...
            const ignoreResourceCalendar : boolean                            = (yield this.$.ignoreResourceCalendar) ||
                ...
                (yield * this.hasElapsedDuration()) ||
                // when we DON"T skip non-working time for manual tasks we use defaultCalendar
                // have to do that for resources too
            ...
            for (const assignment of assignments) {
                ...
                if (resource) {
                    // If the resource calendar is meant to be ignored - use the event calendar instead
                    const resourceCalendar : BaseCalendarMixin  = yield (ignoreResourceCalendar ? this.$.effectiveCalendar : resource.$.effectiveCalendar)
        ...
        * skipNonWorkingTime (date : Date, isForward : boolean = true, iteratorOptions? : Object) : CalculationIterator<Date> {
            ...
            if (yield* this.useEventAvailabilityIterator()) {
                ...
                if (skipRes === CalendarIteratorResult.MaxRangeReached || skipRes === CalendarIteratorResult.FullRangeIterated) {
                    ...
                    const effect = SchedulerProEmptyCalendarEffect.new({
                        event     : this,
                        calendars,
                        date,
                        isForward
        ...
        * calculateProjectedDuration (
            startDate : Date, endDate : Date, durationUnit? : TimeUnit, iteratorOptions?, allowNegative : boolean = false
            ...
            if (yield* this.useEventAvailabilityIterator()) {
                ...
                result = yield* this.convertDurationGen(result, TimeUnit.Millisecond, durationUnit)
        ...
        * calculateProjectedXDateWithDuration (baseDate : Date, isForward : boolean = true, duration : Duration, durationUnit? : TimeUnit, iteratorOptions?) : CalculationIterator<Date> {
            ...
            const durationMS : number               = yield* this.convertDurationGen(duration, durationUnit, TimeUnit.Millisecond)
 ...
 * impossible.
 */
export class SchedulerProEmptyCalendarEffect extends EmptyCalendarEffect {

    getResolutions () : (BaseEmptyCalendarEffectResolution|SchedulingIssueEffectResolution)[] {

        if (!this.getInvalidCalendar()) {
            return [IgnoreResourceCalendarEmptyCalendarEffectResolution.new({ event : this.event })]
        }
...
/**
 * Resolution option for [[SchedulerProEmptyCalendarEffect]] that fixes a specified event scheduling by
 * ignoring its assigned resource calendars.
 */
export class IgnoreResourceCalendarEmptyCalendarEffectResolution extends Localizable(SchedulingIssueEffectResolution) {

    static $name = 'IgnoreResourceCalendarEmptyCalendarEffectResolution'

    event : SchedulerProHasAssignmentsMixin

    getDescription () : string {
        const { event } = this

        return format(this.L('L{descriptionTpl}'), event.name)
    }

    /**
     * Resolves the [[event]] scheduling by ignoring assigned resource calendars.
     */
    resolve () {
        this.event.ignoreResourceCalendar = true
    }
