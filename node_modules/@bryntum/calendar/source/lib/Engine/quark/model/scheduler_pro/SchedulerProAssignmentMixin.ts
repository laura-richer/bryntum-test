    ...
    class SchedulerProAssignmentMixin extends base {
        ...
        * calculateEffort () : CalculationIterator<Duration> {
            ...
            if (event && resource) {
                ...
                const manuallyScheduled : boolean      = yield event.$.manuallyScheduled
                const ignoreResourceCalendar : boolean = (yield event.$.ignoreResourceCalendar) ||
                    ...
                    (manuallyScheduled && !event.getProject().skipNonWorkingTimeInDurationWhenSchedulingManually)
                const eventCalendar = yield event.$.effectiveCalendar

                if (startDate && endDate) {
                    const map  = new Map()
                    if (ignoreResourceCalendar) {
                        map.set(eventCalendar, [this])
                    }
                    else {
                        map.set(resourceCalendar, [this])
                    }
