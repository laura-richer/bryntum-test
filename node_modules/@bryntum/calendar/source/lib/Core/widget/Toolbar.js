...
const
    ...
    widthAffecting    = {
        ...
        minWidth : 1,
        width    : 1,
        disabled : 1 // Need to pass Toolbar.t.js t.it("Should correctly convert button w/menu when overflowed")
    },
    _axisProps        = [{
        box            : 'hbox',
 ...
 *
 * You can let users reorder the toolbar items by setting {@link Core/widget/Toolbar#config-enableReordering} to `true`.
 * {@note}This is not supported if you use `weight` to order the items.{/@note}
 *
...
export default class Toolbar extends Container.mixin(Toolable) {
    ...
    static configurable = {
        ...
        toolDefaults : {
            overflowMenuButton : {
                ...
                icon      : 'b-icon-menu',
                menuIcon  : null,
                ariaLabel : 'L{Toolbar.showOverflowingItems}',
            ...
            overflowScrollEnd : {
                ...
                hidden    : true,
                ariaLabel : 'L{Toolbar.scrollForMore}',
    ...
    updateEnableReordering(enable) {
        ...
        if (enable) {
            me.dragHelper = new DragHelper({
                ...
                lockY             : true,
                dragThreshold     : 10,
                targetSelector    : '.b-widget',
                floatRootOwner    : me,
                rtlSource         : me,
                outerElement      : me.contentElement,
                containers        : [me.contentElement],
                internalListeners : {
    ...
    syncOverflowMenuButton(overflowItems) {
        ...
        for (const item of overflowItems) {
            me.overflowItemFilter(item) && item.addToOverflowMenu(
                menu, me.horizontal ? twinOverridesHorz : twinOverrides, onCreateTwin);
        }

        // Assign each overflow item a weight based on the position of its true self in the toolbar.
        menu.items.forEach(it => {
            let toolbarItem = it._overflowTwinOrigin,
                weight      = toolbarItem.parent.indexOfChild(toolbarItem);

            // In some cases, such as a buttongroup, the twin origin is a grandchild of this container, so fractionalize
            // the position of the grandchild in its sibling list and add on the index of the parent in the toolbar's
            // sibling list
            if (toolbarItem.parent !== me) {
                weight = me.indexOfChild(toolbarItem.parent) + weight / 1e5; // the divisor is arbitrary
            }
        ...
        if (overflowMenuButton._menu?.isMenu) {
            const
                ...
                { toAdd, toRemove } = ArrayHelper.delta(menu.items, existingMenu.items, true);

            existingMenu.remove(toRemove);
            // this works for getting new items in the correct position, even if they were added as items in a
            // buttongroup or just recently new overflows.
    ...
    onLeafItemConfigChange(origResult, { name, value }) {
        ...
        if (toolbar?.isPainted && item.ref !== 'overflowMenuButton') {
            // Do not perform a forced synchronous layout on value change of input fields.
            if (!((item.isTextField || item.isSlider || item.isCheckbox) && name === 'value' && (item.containsFocus || overflowTwin?.containsFocus))) {
    ...
    onBeforeDragStart({ context, event }) {
        const
            item = context.item = this.items[context.element.dataset.itemIndex];

        context.oldIndex = Number.parseInt(item.element.dataset.itemIndex, 10);
         ...
         * @event beforeItemDragStart
         * @param {Core.widget.Toolbar} source The toolbar instance.
         * @param {Core.widget.Widget} item The dragged item.
         * @param {Event} domEvent The browser event.
         * @preventable
         */
        return this.trigger('beforeItemDragStart', { item, domEvent : event });
    }
    ...
    onDragStart({ context, event }) {
        ...
        context.dragProxy.style.backgroundColor = globalThis.getComputedStyle(this.element).backgroundColor;
        /**
         ...
         * @param {Event} domEvent The browser event.
         */
        this.trigger('itemDragStart', { item, domEvent : event });
    }

    onItemMove({ context, event }) {
        const
            { item } = context,
        ...
        this.trigger('itemDragMove', { item, domEvent : event });
    }
    ...
    onDrop({ context, event }) {
         ...
         * @param {Core.widget.Toolbar} source The toolbar instance.
         * @param {Core.widget.Widget} item The dragged item.
         * @param {Boolean} valid `true` if the drop was valid.
         * @param {Event} domEvent The browser event.
         */
        me.trigger('itemDrop', { item, domEvent : event, valid });
    }
