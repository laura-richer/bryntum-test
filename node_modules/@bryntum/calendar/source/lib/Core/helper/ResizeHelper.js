...
export default class ResizeHelper extends Events(Base) {
    ...
    getY(element) {
        if (this.positioning === 'inset') {
            return parseFloat(getComputedStyle(element).insetBlockStart, 10);
        }
    ...
    setY(element, y) {
        if (this.positioning === 'inset') {
            element.style.insetBlockStart  = y + 'px';
        }
        else {
    ...
    initResize() {
        ...
        if (me.grab) {
            ...
            const
                elementStartX = me.getX(element) || element.offsetLeft,
                elementStartY = me.getY(element) || element.offsetTop;
            ...
            me.context = {
                ...
                elementStartX,
                elementStartY,
                newX          : elementStartX, // No change yet on start, but info must be present
                newY          : elementStartY, // No change yet on start, but info must be present
    ...
    update(event) {
        ...
        if (constrainToParent) {
            ...
            context.currentX = Math.max(Math.min(event.clientX, parentRectangle.right), parentRectangle.x) + me.scrollLeft;
            context.currentY = Math.max(Math.min(event.clientY, parentRectangle.bottom), parentRectangle.y) + me.scrollTop;
        }
        else {
            context.currentX = event.clientX + me.scrollLeft;
            context.currentY = event.clientY + me.scrollTop;
        }
    ...
    grabResizeHandle(isTouch, event) {
        ...
        if (element) {
            ...
            const mutatedContext = me.internalBeforeStart?.({ element, edge });
    ...
    updateResize(event) {
        ...
        const
            ...
            deltaX    = (context.currentX - context.startX) * (me.positioning === 'inset' && me.isRTL ? -1 : 1),
            ...
            sign      = (edge === 'right' && (!me.isRTL || me.positioning === 'inset')) || edge === 'bottom' ? 1 : -1,
        ...
        if (me.direction === 'horizontal' && elementWidth !== width) {
            ...
            if (edge === 'left' || width < 0) {
                ...
                if (!skipTranslate) {
                    me.setX(element, Math.round(newX));
                }
    ...
    abortResize(event = null, silent = false) {
        ...
        if (me.direction === 'horizontal') {
            ...
            if (context.edge === 'left' || (context.allowEdgeSwitch && !context.skipTranslate)) {
                me.setX(context.element, context.elementStartX);
            }
            context.element.style.width = context.elementWidth + 'px';
        }
        else {
            me.setY(context.element, context.elementStartY);
